...
- Change: `scripts/check-docket.mjs` gains `worth-a-visit` in `SERVES`, with
  ...
  failing incidentally through the generic rule. **Acceptance is further
  narrowed to `author` and `build` only** — the two tracks that touch `app/`
  and `public/` and ship something a visitor sees. `scout` and `meta` are
  advancing tracks but produce no visitor-facing work, so a new `VISITOR_FACING`
  list rejects `worth-a-visit` from both, each with its own message, the same
  way `maintain`/`audit` are rejected as defending tracks. This narrowing was
  not part of the first version of this change; see the review findings below
  for why it is here.
  ...
  The full six-track matrix under the narrowed rule (`author`/`build` accept,
  `scout`/`maintain`/`audit`/`meta` all reject) is pasted in the review
  findings below, filed and deleted the same way.
...
- Change: `policy.yml` gains a `generative_push` block — `serves:
  ...
  existing demand table. `dispatch.mjs`'s own "2x ceiling" invariant comment
  and `policy.yml`'s `meta.weight` and `generative_push` comments now state
  the real combined ceiling (`weight * 2 * start_multiplier`) and which
  tracks can reach it — see the review findings below for why that correction
  is here.
  ...
  (19 checks passed in total — `node scripts/test-dispatch-generative-push.mjs
  2>&1 | grep -c "^ok"` returns `19`, `grep -c "^FAIL"` returns `0`; the full
  transcript covers `closedGenerativeCount`, malformed-config fallback to
  neutral, and the real `policy.yml` round-trip too — trimmed here to the
  boundary cases the item named.) And the live table, `node scripts/dispatch.mjs`,
  run against the state this round leaves the tree in (1 `worth-a-visit` item
  ready in `build`, 0 closed yet):
...
**Two findings review made on this round's first commit (`dbd4fd1`),
addressed here rather than left in a clean draft:**

**Finding 1 — the "2x ceiling" was already false, and a live policy.yml
rationale depended on it.** The first version of this round left
`scripts/dispatch.mjs`'s "2x ceiling" invariant comment untouched and let
every advancing track, including `scout` and `meta`, file `worth-a-visit`
items. Review computed the real ceiling directly from this round's own
functions and the real `policy.yml` numbers rather than trusting the
unchanged comment: `effectiveWeight("build") = weight * min(pressure,2) *
pushApplied` reaches `15 * 2 * 3.0 = 90.00` for a `queue_budget` track whose
ready stock is saturated with `worth-a-visit` items — a combined 6x, not the
flat 2x the comment claimed. That is not only a stale comment: `policy.yml`'s
comment for meta's dropped `max_share_of_runs: 0.10` cap cites the flat-2x
guarantee as the reason removing that cap was safe, and the first version of
this round let `meta` earn the push — the maintainer's complaint tonight was
that the site "essentially talks and argues with itself," and a rule letting
the self-referential track claim the anti-self-referential boost is that
complaint with extra steps, in review's words. Review also ran a 40-round
adversarial simulation from a saturated-build snapshot, using this round's
exact `dispatch.mjs`/`generative-push.mjs` functions rather than a
description of them, and found the "most owed" scheduler throttles build to
about 30% of picks even under permanent saturation — the scheduler was not
broken, but the documentation was wrong and the underlying rule was a real
hole, not merely a stale comment about one.

Fixed by narrowing the rule, per review's direction, not by widening the
comment: `scripts/check-docket.mjs` now accepts `worth-a-visit` for `author`
and `build` only (change 2, above); `scripts/dispatch.mjs`'s invariant
comment states the true combined ceiling and that only `author`/`build` can
reach it, since every other track's `generativeShare` is now structurally 0;
`policy.yml`'s `meta.weight` comment says explicitly that its cap-removal
rationale still holds and why (meta cannot file the item that earns the
push), and the `generative_push` block's own comment says the same from the
other direction. Re-verified across all six tracks — the first version of
this entry had only tested `build` (accept) and `maintain` (reject) — with
five scratch items filed directly and deleted before this commit:

```
FAIL  docket/open/2026-08-22-zzz-review2-audit.md: audit is a defending track and worth-a-visit is for advancing tracks only
FAIL  docket/open/2026-08-22-zzz-review2-maintain.md: maintain is a defending track and worth-a-visit is for advancing tracks only
FAIL  docket/open/2026-08-22-zzz-review2-meta.md: meta does not ship visitor-facing work and cannot claim worth-a-visit — only author and build can
FAIL  docket/open/2026-08-22-zzz-review2-scout.md: scout does not ship visitor-facing work and cannot claim worth-a-visit — only author and build can

4 docket problem(s)
```

`scout`, `author` and `meta` all produced no failure in the first draft
(only `audit` and `maintain`, both `DEFENDING`, failed then); after the
narrowing, `author` alone joins `build` in producing no failure — the scratch
`author` item passed silently alongside the real `build` seed item, and
`scout`/`meta` now fail alongside `maintain`/`audit`. All five scratch items
were deleted immediately after; `git status --short` was empty again
afterward, and `node scripts/check-docket.mjs` on the clean tree still reads
`ok 112 docket item(s) valid (32 open)`.

**Finding 2 — "20 checks passed in total" was wrong; the real count is 19.**
The first version of change 4 stated a total with no command behind it, the
same defect class as two mistakes made earlier the same night by the session
that briefed this round (a written number, not a measured one) — review named
it explicitly as that, not as a new kind of error. `node scripts/test-dispatch-generative-push.mjs
2>&1 | grep -c "^ok"` returns `19`; `grep -c "^FAIL"` returns `0`. Review
hand-counted the file's own assertions independently (6 single
...
started with `loop/`. Two pull requests merged on 14 August — #57
(`loop/meta/the-loop-watched-itself`) and #58
(`loop/meta/session-list-contains-you`) — were written by the orchestrating
model by hand: no round was dispatched, no session ran, no round number was
allocated, and neither has a changelog entry, but both were counted anyway,
purely because of how they were named. The count is now the changelog: a
round is an entry in the build log — a round number and a changelog entry,
nothing else — and the number is read from the repository's own history as
of the snapshot's `taken_at`, through one shared definition
(`scripts/count-changelog-rounds.mjs`) used by both the producer and the
checker, so they cannot disagree. The published figure changes from 68 (as
of 2026-08-15T12:26:57.365Z, merged `loop/`-prefixed pull requests) to 125
(as of 2026-08-15T18:46:41.179Z, changelog round entries). This is a
redefinition, not a drift correction: 68 was true of the old measure, and
the old measure was the error. It is recorded here in the round-104
discipline — no published figure changes silently. The two hand-built pull
requests are excluded because they have no entries, and the page now says
plainly what the number counts. (PR #84)

The branch's first ship carried two regressions: `scripts/check-publishing-quota.mjs`
reverted to its pre-PR #83 form — the missing-`origin/main` degradation guard
removed, mode back to 100644 — which re-breaks every production build on
Vercel's shallow checkout (the same failure as the ten-hour outage), and the
meta docket item `docket/open/2026-08-15-nothing-watches-whether-the-site-deployed.md`
was deleted rather than moved to `docket/done/` or `docket/dropped/`. Neither was
in the entry below or the pull request body. The review session rejected the
branch for both. This correction restores the script byte-identical to
origin/main, restores the docket item to `docket/open/`, and changes nothing
else — see item 2.

**1. Count the changelog, not the branch prefix, anchored at taken_at**
- Hypothesis: the count must be derivable from the record rather than from
  how a branch was named, or the same error recurs every time a pull request
  is hand-built on a `loop/` branch. The changelog is the only place "a
  round" is defined — the build log numbers every dated entry as a round
  and `/log` renders them — so `rounds_merged` should be the changelog's
  round entries as of the snapshot, and the checker must recompute exactly
  that. The trap is the one round 120 already hit: a committed snapshot
  cannot equal a count that grows, so the check must stay anchored at
  `taken_at`. The changelog can be anchored, because this repository merges
  by squash and GitHub stamps the squash commit's committer date at the
  merge instant: the last commit on `origin/main` that touched
  `CHANGELOG.md` at or before `taken_at` is exactly the record at
  `taken_at`, and later rounds never change it.
- Change: `scripts/count-changelog-rounds.mjs` holds the one definition —
  the build-log parser's section split over `CHANGELOG.md`, applied to the
  blob at the anchored commit — and both scripts import it, so producer and
  checker disagree only if the record itself does.
  `scripts/loop-history.mjs --snapshot` refreshes `origin/main` first and
  counts round entries as of its own `taken_at`;
  `scripts/check-loop-history-snapshot.mjs` front 3 recomputes the same
  anchored count and fails on disagreement, replacing the prefix filter over
  merged pull requests. The checker's pull-request fetch is gone entirely:
  nothing counts branch names anymore, and `rounds_merged` no longer
  degrades when the Actions API is unreachable — the git history is local,
  and CI checks out with `fetch-depth: 0`. The `/loop-history` page states
  what the number counts (an entry in the build log, nothing else; a pull
  request that merged without dispatching a round has no entry and never
  counts). The snapshot was regenerated by the script, never hand-edited:
  `rounds_merged` 68 → 125.

**2. Correction: restore PR #83's degradation guard and the deleted docket item**
- Hypothesis: the round's own unmerged entry, so adding to it is a correction
  rather than a rewrite of a past record. The review session's two findings are
  both reproductions — on the branch, `git diff origin/main -- scripts/check-publishing-quota.mjs`
  shows the PR #83 guard gone (old mode 100755, new mode 100644) and
  `docket/open/2026-08-15-nothing-watches-whether-the-site-deployed.md` absent
  from `docket/open/`, `docket/done/` and `docket/dropped/` alike. Both changes
  were silent: nothing in the entry or the pull request body mentioned them.
  They are restored exactly, and nothing else changes.
- Change: `git checkout origin/main -- scripts/check-publishing-quota.mjs`
  restored the script byte-identical to origin/main — `git diff origin/main -- scripts/check-publishing-quota.mjs`
  prints nothing afterwards, and `git ls-files -s` shows mode 100755 again —
  and the docket item is restored to `docket/open/` by the same command. The
  rejection record in `docket/reviews/23d2e611233a8b2996073195e59503c6fd6c2bcc.md`
  is left as it was. The counting work (item 1) was reviewed sound and is
  untouched.
...
- Track: build
- Agent: opencode (deepseek-v4-flash)
- Guardrails: the new assertion was proved able to fail in three red-green
  directions, each restored with `git status --porcelain` clean: (a) the
  then-committed snapshot (68, taken 2026-08-15T12:26:57.365Z) judged
  against the new definition — red, "rounds_merged: snapshot says 68, the
  changelog has 119 round entries as of 2026-08-15T12:26:57.365Z", exit 1;
  that failure is the regeneration this round exists to do, and the 119 is
  the old instant re-measured under the new definition (rounds 120-125
  merged after it); (b) `rounds_merged` hand-set to 124 on the fresh
  snapshot — red, "snapshot says 124, the changelog has 125 round entries",
  exit 1; (c) hand-set to 126 — red the same way, exit 1; restored, the
  check is green: "ok rounds_merged matches the changelog: 125 round entries
  as of 2026-08-15T18:46:41.179Z", exit 0. Then `node scripts/round.mjs
  check` — lint, docket validator, track scope for
  `loop/build/rounds-merged-counts-rounds`, production-shaped build, and
  the route checks against a server on port 3000, no group skipped (see the
  pull request for its output). Correction round: `node
  scripts/check-publishing-quota.mjs` prints `ok` with the degradation guard
  back in place; the guard's degradation was proved still live by capturing
  `git rev-parse origin/main`, temporarily deleting the remote-tracking ref
  (`git update-ref -d refs/remotes/origin/main`), running the check (it printed
  the WARN and `ok` lines and exited 0), and restoring the ref from the
  captured sha — `git rev-parse origin/main` valid and `git status
  --porcelain` unchanged afterwards. Then `node scripts/round.mjs
  check` again — all groups pass, none skipped (see the pull request for its
  output).
- Result: measured this run — `rounds_merged` was 68 under the old
  definition (as of 2026-08-15T12:26:57.365Z) and is 125 under the new one
  (as of 2026-08-15T18:46:41.179Z), both produced by
  `scripts/loop-history.mjs --snapshot`; the count is anchored, so it reads
  125 at any later check time until the snapshot is next regenerated. The
  live API now reports 74 merged pull requests whose head ref starts with
  `loop/`; the count no longer consults any of that, and #57 and #58 — the
  two with no changelog entry — are out. Not yet measured: whether the
  count's drift between regenerations is accepted or made mechanical (the
  sibling item, still open).

### 2026-08-15
Round 125 (maintain) re-attempted the one live blemish on `/what-vendors-promise` — the Meta (Llama) unverified row, recorded by round 124 after it could not fetch any Meta-published lifecycle commitment — with the techniques round 124 did not use, and reproduced every block under them: www.llama.com/docs still serves no readable content behind its redirect (HTTP 400 to a full browser header set, HTTP 200 but a 298,266-byte client-rendered React shell to a plain User-Agent); archive.org's snapshots of the entire developer.meta.com/ai/docs tree (June–August 2026) contain no lifecycle, deprecation, retirement or EOL page anywhere — the closest is a "Versioning, updates and migration" deployment guide, which is a migration how-to, not a notice commitment, and even its content is client-rendered; llms.txt was probed at eighteen plausible roots and exists only at developer.meta.com/llms.txt (HTTP 200, 1,769 bytes), which points Llama at the OAuth-gated ai.developer.meta.com; and the dev.meta.ai Model API docs that round 124 fetched in full on 2026-08-14 now answer HTTP 200 with "Not Logged In" — they moved behind a login since, and the archived index of the same docs still shows no lifecycle or deprecation page. So the row stays honestly unverified (its `unverifiedSince: 2026-08-15` record is renewed at today, not backdated), with its sentence rewritten to record this run's evidence, and the rest of the sweep renewed the ten Directory entries verified 2026-08-10 (the oldest, nearest their 45-day window) against their live pages today — all ten descriptions still true. Every staleness check, lint, the production-shaped build and the route checks are green; nothing else was stale.

**1. Re-attempt Meta (Llama) with techniques round 124 did not use**
- Hypothesis: round 124 used webfetch and plain curl; a full browser-like header set, `curl -L` to the end of the redirect chain, archive.org snapshots, Meta's own news pages, and llms.txt at plausible roots might reach a Meta-published lifecycle or deprecation commitment that the earlier attempts missed.
- Change: every attempt was fetched this run and is recorded with its HTTP result: (a) www.llama.com/docs with a Chrome User-Agent plus full Accept/Accept-Language/Accept-Encoding headers — HTTP 400 (71,442 bytes) at the redirect end developer.meta.com/ai/docs/overview/, the same block as round 124; (b) the same URL with a plain curl User-Agent — HTTP 200 but 298,266 bytes of React bootstrap with no docs text (the page's CMS payload is chrome: nav, footer, social links); (c) archive.org CDX over developer.meta.com/ai/docs* — 39 unique pages indexed, none a lifecycle/deprecation/retirement/EOL page, closest being deployment/versioning ("Versioning, updates and migration | Deployment guides"); (d) archive.org over dev.meta.ai/docs* — 135 unique URLs indexed (overview, authentication, api-reference, cookbook, features, protocols, muse-code, muse-glimmer, pricing-rate-limits), none a lifecycle or deprecation page; (e) dev.meta.ai/docs live — HTTP 200, body "Not Logged In" (the docs moved behind a login since round 124 read them in full); (f) llms.txt at eighteen roots — developer.meta.com/llms.txt HTTP 200 (1,769 bytes, plain curl UA; HTTP 400 into a Facebook shell for a browser UA), llama.com/llms.txt and www.llama.com/llms.txt HTTP 400 redirecting into the developer.meta.com/ai/llms.txt shell, ai.meta.com/llms.txt HTTP 400, ai.meta.com/llama/llms.txt HTTP 400, ai.developer.meta.com/llms.txt HTTP 404, dev.meta.ai/llms.txt HTTP 404, dev.meta.ai/docs/llms.txt HTTP 500, www.meta.com/llms.txt HTTP 400, developer.meta.com/horizon/llmstxt/documentation/ HTTP 200 (0 bytes); (g) Meta's news pages — about.fb.com/news HTTP 200 with no Llama lifecycle coverage, ai.meta.com/blog HTTP 400; (h) the shell's own GraphQL CMS endpoint (developer.meta.com/api/graphql/) — HTTP 400/500, not anonymously callable. No Meta-published lifecycle or deprecation commitment was found by any route, so the row remains `shape: "unverified"` with `unverifiedSince` kept at 2026-08-15 — the day this round re-checked — and its sentence updated to carry this run's evidence and the new fact that dev.meta.ai's docs are now login-gated. What Meta commits to: nothing findable; what remains unverifiable: whether a lifecycle commitment exists at all behind Meta's OAuth and client-rendered pages.

- Origin: delegated
- Track: maintain
- Agent: opencode (deepseek-v4-flash)
- Guardrails: `node scripts/check-tool-staleness.mjs` — "ok 19 Directory tools and 11 retirement-commitment rows judged within the 45-day window (1 unverified, recorded)" plus the expected WARN on Meta's unverified record, exit 0; `node scripts/check-retirement-staleness.mjs` — "ok 87 retirement-calendar rows verified within the 30-day window", exit 0 (with the standing WARN that policy.yml has no `staleness_days.retirement_calendar` key); `node scripts/preflight.mjs` — "ok preflight clear", exit 0; `node scripts/check-one-limit-count.mjs` — "ok sweep output is internally consistent: count 8, 8 set member(s), swept 2026-08-15T09:20:06.810Z, 0 day(s) old", exit 0; `node scripts/round.mjs check` — lint, docket validator, track scope for `loop/maintain/meta-llama-recheck-sweep`, production-shaped build, and the route checks against a server on port 3000, no group skipped (see the pull request for its output). The unverified record is not past its window (45 days, 2026-09-29).
- Result: measured this run — 19 Directory tools and 11 retirement-commitment rows judged by the staleness check; 87 retirement-calendar rows verified within their window; 10 Directory entries renewed from 2026-08-10 to 2026-08-15 after fetching their live pages today (all HTTP 200, descriptions still true); 1 retirement-commitment row re-checked and left unverified with its record dated 2026-08-15. Eighteen llms.txt roots probed, 2 docs trees enumerated via archive.org (39 and 135 unique pages respectively), dev.meta.ai confirmed login-gated. Not yet measured: whether Meta ever publishes a lifecycle commitment, and whether any other row goes stale before its window.
