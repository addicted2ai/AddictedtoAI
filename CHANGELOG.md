...
- Change: measured this round, one production build per commit, `curl -H
  ...
  the budget check in `scripts/check-routes.sh` measures `/log` at 90,326
  bytes gzipped — 56,674 bytes of headroom against the 147,000 local
...
- Guardrails: `node scripts/round.mjs check` — every check passed, including
  the budget line for `/log` (90,326 bytes gzipped, 56,674 to spare) with
...
- Result: `/log` 90,326 bytes gzipped with this round's entry rendered,
  56,674 under the 147,000 local ceiling; measured by the budget check in
