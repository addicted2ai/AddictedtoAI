...
5. `scripts/check-frame.mjs`'s completeness guarantee went through two fixes that stayed inside shape-recognition and still left a case unhandled, before a third replaced the approach itself with a declared, checkable total, and a fourth closed a remaining flaw in that new approach (a count could match the wrong set) — the precedent this round's own `scripts/check-briefs.mjs` follows rather than re-derives. [command: grep -c "COMPLETENESS DOES NOT COME FROM RECOGNISING HEADING SHAPES" scripts/check-frame.mjs]
