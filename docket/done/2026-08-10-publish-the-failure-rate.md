...
- [ ] The site publishes attempted, succeeded, and failed run counts alongside
      rounds shipped
- [ ] The figures are derived, not typed — and the derivation does not make a
      network call during the build, or degrades cleanly when it cannot
- [ ] A run that failed is distinguishable from a run that correctly found
      nothing to do (rule 20), because conflating them would misrepresent both
- [ ] Whatever snapshot the page reads carries the date it was taken, so a
      stale figure is visible as stale rather than silently wrong
- [ ] The check was shown to fail: feed it a snapshot claiming zero failures
