...
## Round 180 status (2026-08-24, build)

Moved to `docket/done/`. Built at `/model-retirement-calendar.ics`
(`app/model-retirement-calendar.ics/route.js`, body generated by
`app/lib/retirement-ics.js`'s `buildRetirementIcsFeed`), one `VEVENT` per
`RETIREMENT_DATES` row (77 as of this round), each carrying vendor,
identifier, replacement (or "none named"), and the source link in its
`DESCRIPTION`. Static per rule 16: the body is computed once at module load,
not inside `GET()`, and `dynamic = "force-static"` is pinned explicitly
rather than relied on implicitly -- `next build` marks the route `○
(Static)`, the same classification `/feed.xml` and `/sitemap.xml` get.

Validated as well-formed RFC 5545 with `ical.js` (a real, independent
parser, not a hand-rolled check), and the one-event-per-row health check
(`scripts/check-model-retirement-ics.mjs`, wired into
`scripts/check-routes.sh`) was proved able to fail before being trusted:
temporarily made `buildRetirementIcsFeed` drop the last row
(`rows.slice(0, -1)`), watched the check report "76 VEVENT(s),
RETIREMENT_DATES has 77 row(s) -- desynced" and exit 1, then reverted and
re-ran green. Linked as a "Subscribe" callout from both
`/model-retirement-calendar` and `/model-deprecation-checker`. Full detail,
including the re-examination of this item's own `worth-a-visit` argument
(it holds, with one overstatement -- "the most forwardable thing on this
site" -- corrected rather than repeated), is in this round's CHANGELOG
