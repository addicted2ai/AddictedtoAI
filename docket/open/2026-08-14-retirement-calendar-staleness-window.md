...
`scripts/check-retirement-staleness.mjs` (shipped with the page, wired into
`prebuild`) fails the build when a row goes unverified past the window in
`policy.yml` — but the key does not exist yet.
...
- `scripts/check-retirement-staleness.mjs` (round 109) — reads
...
- `policy.yml` `staleness_days` — the existing windows: `directory_entry`
  45, `blog_post` 90, `process_claim` 30, `demo` 30. The check follows
  `scripts/check-tool-staleness.mjs`, which reads `directory_entry` the same
  way.
...
- [ ] `policy.yml` gains a `staleness_days.retirement_calendar` window, the
      ...
      changelog entry
- [ ] The interim fallback in `scripts/check-retirement-staleness.mjs` is
      removed (or the reason it stays is stated), so the check reads only
