...
- [ ] A staleness check reads `app/lib/retirement-commitments.js` and fails the
      build when a verified row goes unverified past its window, in the shape
      of `scripts/check-tool-staleness.mjs`
- [ ] An unverified row (a vendor whose page could not be fetched) is handled
...
- [ ] The window the check reads is in `policy.yml`, added by the track that
      owns it, with the trade between a shared and a dedicated key argued
- [ ] Proved able to fail: a row with an old `verified` date trips the check,
