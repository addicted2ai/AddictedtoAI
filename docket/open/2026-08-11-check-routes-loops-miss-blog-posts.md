...
(around line 83) and the document-size budget loop (around line 121). Both are
hardcoded to the same ten routes:

```
/ /blog /blog/frontier-cyber /directory /demos /log /log/archive /projects /disclosure /charter
```

Three published blog posts are absent from both: `/blog/claude-code-auto-mode`
(round 80), `/blog/cyber-eval-cascade` (round 82), and now `/blog/gpt-5-6-price-drop`
(round 84). Rounds 80 and 82 both hit this, recorded in each round's entry as a
residual gap "left for a later round or the maintainer". This round is the
third instance, and it is the round that files the item rather than noting it
again.
...
round, and `check-routes.sh` calls it (line 90). So the missing pieces are the
...
- `scripts/check-routes.sh` lines 83 and 121 — the two hardcoded `for route
