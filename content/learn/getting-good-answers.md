...
Ask for a product description in a warm, plain register and you have handed over
a description of a document that does not exist yet. Your adjectives have to be
...
What examples in a prompt mainly supply is the shape of a task rather than the
correct answers inside it, and how far that holds turned out to depend on the
size of the model reading them. The behaviour has a name,
...
summary" describes an enormous one. The model produces the continuation with the
most probability behind it, which lands near the middle of everything that would
have counted, because the middle is where the mass is.

So the flatness people complain about is rarely the model playing safe. It is
the honest centre of a set drawn too wide. Specificity works by exclusion.
"Professional" sits beside almost any prose ever written and rules out nearly
nothing. "Under 150 words,
...
One pass through the network produces one token, and a hard question does not
buy more layers than an easy one. The only way a model gets more computation is
...
## An instruction is only a position in the sequence
...
from being present in every request, never from rank. Nothing in the
architecture protects it, which is why asking a model to override its
instructions is coherent to attempt at all. What stops it sits outside the
model.

Its grip also loosens as a conversation grows, because every position added
competes with every position already there. Your own instructions decay the same
way, and corrections fare worse still, because a correction removes nothing. The
wrong answer stays where it was, the objection lands underneath, and by the
fourth round of "no, not like that" the strongest
pattern in the document is the mistake, restated, with the model's own agreement
stacked on top. Editing the message that started the drift, or starting again
carrying what you learned, removes text. Nothing else available to you does.

## Politeness, tips and threats

Tone changes the reply for a mundane reason: rude text is followed, across
almost everything ever written, by different text than polite text. A 2024 study
across three languages reported that [impolite prompts often result in poor
performance, while overly polite language does not
guarantee better outcomes, and the best politeness level differs by
language](https://arxiv.org/abs/2402.14531). Notice how little that resembles
the rule people repeat.

By 2025 tipping a model was a widely traded tactic, and threatening it had a
public endorsement from a Google co-founder — models tend to do better if you
threaten them, he said. In August 2025 four researchers measured both and
reported that [threatening or tipping a model generally has no significant
effect on benchmark performance](https://arxiv.org/abs/2508.00614).
...
the machinery rather than anyone's care. The last step of producing a token is
a sample drawn
from a distribution, so the same prompt run twice gives different text with
...
token is not optional. The confident empty answer is not a refusal to try
harder. It is the only thing the machinery can do with a question it has
nothing for.
...
Both boundaries have the same two exits, and neither is a better sentence. The
missing thing goes into the input, or the output gets checked somewhere outside
the model. Those change what is present. Rephrasing changes only which
continuation of the same material is likeliest, a narrower power than it feels
like at three in the morning. Stop asking what to say to the model. Ask what
