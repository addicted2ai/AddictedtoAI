...
| `run-tests.mjs` | what `npm test` runs: finds `*.test.mjs` under the source directories and hands them to Node's built-in test runner. Serialises concurrent `npm test` runs over one tree through its own lock (`build-lock.mjs`, `TEST_LOCK_SUFFIX`) — separate from the build lock, because a `pulse` HTTP-fixture test can otherwise fail on `EADDRINUSE` from machine-wide ephemeral-port exhaustion when several `npm test` callers overlap, which reads as a real test defect and is not (`addictedtoai-ngz`) |
| `check-spec-deltas.mjs` | `addictedtoai-vl9` — the pre-archive check on OpenSpec deltas, registered as the `spec-deltas` prebuild step. Fails the build on a `MODIFIED`/`REMOVED`/`RENAMED` heading that resolves to no requirement anywhere; warns on cross-change collisions, archive-order dependencies, change-relative narration and stale identifiers. **Run `node scripts/check-spec-deltas.mjs --strict` before `openspec archive`** — it promotes those warnings to refusals, because archiving is a one-way door into a reserved path |
...
| `seed-frontier-history.mjs` | `separate-a-claim-from-a-fact` task 25 — the ONE-TIME backfill of the lead-change record, replaying the committed `data/sources/<source>/latest.json` blobs through `execFileSync('git', …)` so the surface is not empty on the day it ships. **Run by the orchestrator, by hand — never by a Desk job (a job that seeds its own history writes the evidence it is judged against) and never by the Pulse (a one-time backfill over git history is not state the Pulse recomputes).** Idempotent, appends only, never overwrites an observed line; `--dry-run` prints what it would append. With no metric registered in the registry's `frontier` block it writes nothing and says so, which is the state on 2026-09-06 |
