...
peak_guard() {
  local verdict field_re window window_start resumes_at
  ...
  case "$verdict" in
    ...
    "PEAK "*)
      ...
      if [ -n "${ORCHESTRATE_PEAK_AUTH:-}" ] && [ "$ORCHESTRATE_PEAK_AUTH" = "$window_start" ]; then
        note "PEAK WINDOW $window UTC -- authorised (ORCHESTRATE_PEAK_AUTH matches this window's start $window_start; reason: ${ORCHESTRATE_PEAK_REASON:-none given}) -- starting this iteration at double rate (deepseek-v4-flash \$0.44/\$1.32 per 1M vs \$0.22/\$0.66 off-peak)"
        return 0
      else
        note "PEAK WINDOW $window UTC -- deepseek-v4-flash is double rate here (\$0.44/\$1.32 per 1M vs \$0.22/\$0.66 off-peak) and no matching authorisation is set (ORCHESTRATE_PEAK_AUTH must equal $window_start to start a new iteration now). Not starting a new iteration; resuming automatically at $resumes_at UTC. This pauses the loop, it does not stop it, and does not touch an iteration already in flight."
        return 1
      fi
      ;;
    *)
