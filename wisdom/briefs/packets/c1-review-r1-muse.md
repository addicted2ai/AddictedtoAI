...
6. **START FROM THE WHOLE DIFF, NOT FROM THE PARTS THAT LOOK BEHAVIOURAL.**
   ...
   **AND YOU CANNOT MUTATE, SO DO THE HARDER HALF INSTEAD.** You have no edit
   ...
   - enumerate EVERY changed line in the merge-base diff, by hand, from
     `git diff`, and mark each as behavioural or not — the non-behavioural
     ones still have to appear, and still have to be declared by the author;
