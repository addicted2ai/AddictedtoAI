# Packet B2 — tasks 10–12 of `two-desks-work-orders-and-trains` (author brief, round 6: finish the four arms — finding 3 corrected, finding 4 run)
...
- If a tool call is blocked or refused, record it in `RESULT6.md` and stop
...
Rounds 4 and 5 both stopped on the stop rule, and both times the ARM was
wrong, not the code — the architect has read both and rules so. The stop
rule is REFINED for this round: when an arm goes red against the current,
unmutated production code, first DIAGNOSE it — does the arm match something
it did not mean (text inside the fenced diff, a marker where it expected a
section, a cap that cannot bind)? If the arm is wrong, FIX THE ARM and say
so in `RESULT6.md` with the reason; only a red whose cause is the production
code — the requirement says one thing, the code does another — is a STOP,
reported in §7 with the output, the production file untouched.

State of the four arms in the worktree (uncommitted, in `git status`):
findings 1 and 2 are DONE — round 5 ran their named mutations red and
restored green by hash (RESULT5.md §4-5); carry those rows into your table as
"done in round 5" and do not redo them. Findings 3 and 4 remain.

### Finding 3 (REVIEW3 §3) — the revision brief's section ORDER, `review.test.mjs:379-392` — the arm is wrong, fix it
Round 4's arm locates each section heading with `brief.indexOf(heading)`.
The revision brief's "## Diff under revision" section is a FENCED block
carrying the judged diff, and in this fixture the judged diff contains the
mock job's own brief text, so `## Ground rules (non-negotiable)` matches
first INSIDE the diff (offset 8,737 in round 5's run) before the real tail
(17,469). Fix: locate headings only OUTSIDE fenced code blocks — strip
every ``` … ``` block from the brief before indexing (or index only lines
that begin with `## ` and are not inside a fence), then assert the six
indexes strictly increase: Verdict, Acceptance checks, Diff under revision,
Relevant spec excerpts, Ground rules, How to end. Named mutation:
`loop/lib/brief.mjs:798-812`, the excerpts block moved before the diff block
— it must go red under the corrected arm; restore by hash.
...
Run the targeted group and the two property enforcements; commit the two
test files; the final full-suite iteration on the committed tip; RESULT6.md.
No sweep this round — round 3's enumeration and REVIEW3's seven extra
mutants stand.
...
- `RESULT6.md` — (new) your report, in the worktree root; see "## How to end"
...
## Mutations (perform, observe red, restore; table in RESULT6.md)
...
1. Read `REVIEW3.md`, then `RESULT5.md` and `RESULT4.md` (rounds 4 and 5 stopped on the stop rule; their uncommitted arms are in the tree), then the changed lines each finding
...
5. Commit ONLY the two test files (`git add` each by path; never
   `git add -A`); do not add `RESULT6.md` or `.agent-brief.md`.
...
Write `D:/addictedtoai-worktrees/fleet6-stage0-B2/RESULT6.md` — this exact
