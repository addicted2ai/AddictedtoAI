...
checked out at `5414899`. `node_modules` is a junction to the main checkout —
...
1. Read `loop/lib/select.mjs` whole, `loop/lib/runners.mjs:16-120`,
   `loop/run.mjs` (`runLoop` from line 934: `pickRunner` at `:943`, the
   reviewer at `:944`, the resume branch `:1193-1254`, the `selectJob` call
   at `:1256`, the dry-run return at `:1336`, and every later read of
   `runner` — `:274` the author phase entry, `:303`, `:321`, `:779` the
   revision, `:1472-1474` the outcome line; line numbers at the authority
   commit). THE QUOTED PARAGRAPHS CITE `run.mjs` AT `9c1d980`, before packet
   E moved it: read their `:264`, `:287`, `:305`, `:757`, `:910`, `:911`,
   `:1160-1221`, `:1223`, `:1303` and `:1439-1441` as `:274`, `:303`,
   `:321`, `:779`, `:943`, `:944`, `:1193-1254`, `:1256`, `:1336` and
   `:1472-1474` here; `select.mjs`, `runners.mjs:16-76` and `budget.mjs`
   did not move (measured by diff, `9c1d980..` the authority commit),
