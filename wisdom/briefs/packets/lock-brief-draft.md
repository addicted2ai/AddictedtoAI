...
## THE THREE NEGATIVE CONTROLS, AND THE MIDDLE ONE IS THE POINT
...
Your test must show all three, each as its own case with the observed behaviour
asserted:
...
Add a fourth if you can: **present, fresh, but unreadable or malformed** — the
Pulse must still refuse (mtime decides) and must not throw.
...
and a Pulse that quietly stops working. Do not merge without it.
...
than sleeping, so the suite stays fast and the aged case is exact rather than
approximate.
...
## Your report — `RESULT1.md`
    ...
    ## 3. The three controls, each with the observed output
    | case | lease state | expected | observed | asserted where |
    ## 4. The malformed case, if I built it
