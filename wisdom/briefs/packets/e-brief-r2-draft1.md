...
actually received. That text is committed verbatim as `.job/brief.md` on the
job branch (`run.mjs:1326-1327` writes `briefText` and adds it), so the arm
can read it back: `git -C <ctx.repoRoot> show <result.branch>:.job/brief.md`
(or the same path at `result.mergedSha`) and assert its `.length` EQUALS
`line.brief_chars`. If what git returns differs from `briefText` (line
endings, a trailing newline), measure and say which, and assert against the
committed bytes' true length rather than loosening. Named mutation: the
reviewer's `brief_chars: 1` at `run.mjs:1484` goes red; restore.
...
`carry:` list (two entries) and an assertion that the `review1` phase
records `carried: 2`, keeping the zero and the no-record cases.
`loop/tests/corrections.test.mjs` and
`loop/tests/discarded-proposal-retry.test.mjs` carry `carry:` in fixture
records — read them for a reusable reviewer pattern first; if none fits,
add ONE mode to `loop/tests/mock-executor.mjs` (`review-approve-carry`,
beside `review-approve` at `:262`) that writes the same approve record plus
two `carry:` entries, and use it. Named mutation: the reviewer's
`carried: 0`-for-every-verdict at `run.mjs:678` goes red; restore.
