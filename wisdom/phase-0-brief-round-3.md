> **PROVENANCE.** Round 3's revision brief, and the ruling in it is the part
> that transfers: binding all thirty-four misses has no terminating condition,
> because arms multiply faster than fixtures. **The unit of work is a
> DISPOSITION, not a test** — BIND, DELETE, or RECORD with a named owner and an
> expiry — and the sweep ends when every arm has one.
