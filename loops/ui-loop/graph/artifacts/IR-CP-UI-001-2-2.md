...
## Gates
- build PASS (log clean; only pre-existing `currency-literal` content warnings; shared JS 103 kB)
...
capabilit·claim·role·description·verified·accuracy·swe·agent) — typed at the source, not a
stop-list. No org doc is read here by any path. 4 of 16 rows carry a claim (Google DeepMind,
Inception Labs, SpaceXAI, Tencent); 12 render the blank.
**Hatched: 12 of 96 value cells at 1440 (12 in the first viewport) and 12 at 390 (10 in it).**
No founding date, founder or company fact anywhere on /frontier — asserted, not eyeballed: S22(a)
reads every `cited` fact out of `content/wiki/org/*.md` and fails if one appears in the page.
...
ellipsis`; the board is then 1152px in a 1152px shell at 1440 — READ on screen, no container
scroll. The >90% state the gate found was not the claim column (after fix 1 it
runs 75/25) but **READ, repeating `openrouter-models · 2026-09-05` on 16 of 16 rows at the price
columns' ink weight**. Answered with /catalog's own mechanism, not a second one: `renderFetchLine`
reused verbatim above the board, unexceptional rows `--muted` via `data-default`.
`renderPlayersBoard` still emits `p.board-note` if a claim state ever clears 90%. No copy authored
— both strings are the cell's own.
Δ `frontier--light--1440.png`: 16 chips, claim cut mid-word, READ off-screen → no chip, a fetch
line above, one-line ellipsised claims, READ inside the shell.
...
— the board's own treatment, not a look-alike — over a row rule matching `#frontier-board tbody
tr`, which the board now carries because R8's surface test (16×7) requires it. No per-row rules
(AR-001 D3). 390 is the nav item (K25), now asserted by S24.
Δ `home--light--1440.png` door: Alibaba Cloud/Anthropic/Cohere (alphabetical, frozen — RT FM3) →
Alibaba Cloud/OpenAI/Mistral AI, at the board's lead weight over its row rule.
...
its 292px tracks and 44.9% are exactly F-struct-2/3's numbers (a check written against the literal
selector would be vacuous). `.rails` becomes one `fit-content` flow — R13's other permitted answer;
letting the shorter rail set the row is unavailable, list lengths being data. Each rail takes
...
code` untouched — inside a scrollport, R2's own remedy). Widening is NEW invariant S23: ONE ROUTE
PER TEMPLATE (18) at 390 and R2's own 320; `scripts/verify-design.mjs` not touched.
...
`.listing` — not `anywhere`, which also shrinks min-content, and these rows sit in the subgrid
track set S19 holds to a 200px floor.
...
reordering columns is the board grid, and the cap achieves the invariant instead.
...
- **R2 + S23** NEW. `.prose code{overflow-wrap:normal}` fired at 469px on /tutorials/<entry>
  (verdict 465px); scope end `body{min-width:1200px}` fired on the first sampled template.
- **R6 + S24** NEW (flagship reachable at 390 AND feed still leads). Hiding the nav item fired;
  ...
  reachability-only check would score as a win.
...
- **R10 + S5** widened to `.rails` → "292.0px, 160.1px from its own widest child" (verdict
  162.0px); opposite sign 60.0 vs 102.6px. **R13 + S18** second clause (one flow, or 60%) → 52.4%;
...
every item to the row height, so an item-BOX ratio there is 100% by construction. Rewritten to
measure OCCUPIED height — S1's own vacuous-box mistake, caught only because the falsifier ran.
(3) S22 (c)'s break removed the cap, which overruns the TABLE, not the CELL. (4) S22b's opposite
widened `.board-wrap` alone; the table is `width:100%` of it and tracked the new edge. Also, S5's
clause first used `.rails > section` — matching nothing.
...
Untouched: rig, `loops/ui-loop/evidence/`, `content/`, `data/`, `public/`, K6 reserved paths.

## Notes for verify
- K23–K29 are not in `loops/ui-loop/state.md` (it stops at K22); they arrived in the dispatch.
- /frontier at 390 still scrolls the BOARD inside `.table-wrap` (1148px in 362px) — R2's own
  remedy and R12's expectation, not a defect; S22b declares no opinion at 390.
