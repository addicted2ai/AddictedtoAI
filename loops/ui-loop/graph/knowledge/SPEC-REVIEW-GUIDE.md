...
| `frontier: true` requires `frontier_reason` ∈ F1–F5 and ≥1 `domains` | K30, tightened in discussion | The cap exemption is a loophole unless the flag has a bar; the domain is what lets the section auto-populate. | — |
...
| Seeded values machine-maintained (beside `timeline`); editorial assignments a separate field via review | K44 | 545 wiki files; one field must not carry two freshness regimes (orchestrator's review-hash finding). | — |
...
| Org entries list product-brand domains as aliases | §2 amendment | Missing brand domain renders a real claim as an honest-looking blank (RT FM-N6). | — |
