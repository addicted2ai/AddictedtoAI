...
| `openspec validate <change-name> --type change --strict --no-interactive` | the spec artifacts of one unarchived change. Name a change that is still in `openspec/changes/`; an archived one reports "No deltas found". |
...
This project is indexed by GitNexus as **AddictedtoAI** (19808 symbols, 39271 relationships, 357 execution flows).
...
| Resource | Use for |
| --- | --- |
| `gitnexus://repo/AddictedtoAI/context` | Codebase overview, check index freshness |
| `gitnexus://repo/AddictedtoAI/clusters` | All functional areas |
| `gitnexus://repo/AddictedtoAI/processes` | All execution flows |
| `gitnexus://repo/AddictedtoAI/process/{name}` | Step-by-step execution trace |
