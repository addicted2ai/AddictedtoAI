...
|---|---|
| `ifm-k2-horizon-open-fleet.md` | **F3 fails on both branches.** First branch needs a *covered organisation*: the Institute of Foundation Models at MBZUAI has **no `content/wiki/org/` entry** (24 entries exist; the transcript lists them) and no catalog presence — a `grep -ric` for `mbzuai\|ifm-ai\|"ifm"\|k2-horizon` across `data/derived/` and `content/wiki/` returns exactly one hit, `search-index.json`, which is this post indexing itself. Second branch needs "an open-weights release matching a covered lab's frontier **on a published measure**", and the post cites none: its only cross-vendor figures are Artificial Analysis's *reward-hacking flag rates* for Claude Fable 5 (2.2%) and GPT-5.6 Luna (4.1%), which are not capability scores. F5's "a **frontier** model … opened" rests on IFM's own page title ("Frontier Performance, Radically Open") and the post declines to endorse it, labelling the numbers vendor-reported. **Refile condition (widened 2026-09-06 by job j-20260906-18, carried finding `j-20260906-17-carry-1`):** if an IFM/MBZUAI org entry lands under §2's coverage widening, this post is re-judged **against all five criteria on their own terms**, not against F3 alone. The trigger is F3-shaped because F3's first branch is the one an org entry mechanically unblocks; the re-judgment is not, because F4 also carries a coverage-shaped clause ("a major player") and F5 carries none at all, so an F3-shaped trigger must not be allowed to decide which criterion gets looked at. F4 and F5 were judged on their own terms on 2026-09-06 and both still fail — the reasons, which coverage does not settle, are in `verify-frontier-backfill-recheck.md`. It is the strongest decline in the set. |
