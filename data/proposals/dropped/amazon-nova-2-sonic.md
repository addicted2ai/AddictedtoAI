...
in audio: **Amazon Nova 2 Sonic**, a native speech-to-speech model in the
Nova 2 series, positioned for fast natural conversation.

## Which test it failed, and why

**Worth a stranger's attention.** It is a point release of an existing
product line announced through a vendor product blog. It is correct, sourced
and forgettable, which the bar names as a failure rather than a near miss.
No artifact, no measured comparison, no access change, and nothing a reader
could act on that the vendor's own page does not already say better.
...
artifact. Speech-to-speech is an established capability; this is a better
version of it.

**F3** — it is not positioned by Amazon as Amazon's frontier model, and it is
not an open-weights release matching a covered lab's frontier on a published
measure.
...
shows does not qualify on its own. Recording that the audio domain was swept
...
- A measured latency or interruption-handling figure from a third party,
  since that is the only claim in the category that a reader cannot verify by
  looking and where a real number would be worth sending.
