...
Assembled context — the 97 change-feed line(s) from the trailing 7 days that no published post's `covers:` declares. A mechanical join of `data/changes.jsonl` against those declarations, in the feed's own order: no score, no shortlist, and no claim that any of it is worth writing about. It is what the site already knows, offered as an input — the search is the world beyond this repository, and these lines do not bound it.
...
### PENDING AMENDMENT to `specs/loop` — in-flight change `let-dated-news-outrank-the-queue`
(full text: `D:/AddictedtoAI/openspec/changes/let-dated-news-outrank-the-queue/specs/loop/spec.md`)
