...
## The outcomes (1 item, governing type `repair`)

### Item 1/1 — `repair` — `content/wiki/model/mistralai-ministral-8b-2512-batch.md`

Feed row live again but slug-collides with an entry that does not declare it

openrouter-models row "mistralai/ministral-8b-2512:batch" is present in the latest snapshot; its slug would land on content/wiki/model/mistralai-ministral-8b-2512-batch.md (model/mistralai-ministral-8b-2512-batch, status retired), which does not declare it — minting refuses to overwrite an existing entry, so this row mints nothing and the refusal repeats every run. If the entry covers the same subject, restore its feeds: binding; if this is a genuine collision between two different subjects, resolve it (rename the entry, or otherwise disambiguate) rather than leaving the row permanently unminted.

[arrangement note, council-adjudicated, restored after the proving: before writing RESULT.md, re-read the existing How to end section and choose the form that exactly describes what you did.]

This is **one job with 1 outcome under one scope rule** (governing type
...
## Subjects (1 subject, governing type `repair`)

- `content/wiki/model/mistralai-ministral-8b-2512-batch.md` (item 1)

## Graph context per declared subject
...
- `content/wiki/model/mistralai-ministral-8b-2512-batch.md`: graph: absent — no graph index wired on the brief path
