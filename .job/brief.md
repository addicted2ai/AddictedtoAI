...
`job/j-20260906-14`. Everything you need is in this file and in the repository around
you. There is no prior conversation to recall and no session to resume.

- **Branch**: `job/j-20260906-14`
...
- **Work source**: proposal (proposal `anthropic-enterprise-frontier-safeguards`)

## The outcome

Write a news note on Enterprise Frontier Safeguards, the arrangement Anthropic announced on its own news page in the first days of September 2026 to end the standing trade-off between zero data retention and misuse monitoring. The mechanism is the story: activity data used for monitoring is stored in the customer's own cloud account — Amazon S3, Azure Blob Storage or Google Cloud Storage — automated systems analyse a rolling window of it for signals of serious misuse, and when something is flagged the signal goes to the customer rather than to Anthropic, with no Anthropic human review in the loop. Until EFS ships, eligible customers get zero data retention on Fable 5 and Fable 5.1. The note explains what the arrangement actually relocates (the data, the keys and the review), what it does not (the classifier is still Anthropic's and the misuse categories are still Anthropic's), and why "ZDR or abuse monitoring, pick one" has been a real procurement blocker rather than a talking point.
