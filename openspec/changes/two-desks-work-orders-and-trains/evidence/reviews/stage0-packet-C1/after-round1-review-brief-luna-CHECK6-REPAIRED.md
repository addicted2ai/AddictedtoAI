...
judge and show your evidence. You have NO EDIT RIGHTS to any tracked file: do
not commit, do not push, do not modify the implementation or its tests except
as a MUTATION you restore immediately and prove restored by hash.
...
5. **AND THE SAME SENTENCE'S SECOND ERROR:** the brief text must no longer call
   ...
   narrowing, say so.
6. **START FROM THE WHOLE DIFF, NOT FROM THE PARTS THAT LOOK BEHAVIOURAL.**
   `git diff <merge-base>..<tip>` is the complete list of what changed, and
   every hunk in it must be accounted for in your report — production lines,
   test assertions, test FIXTURES, comments, documentation strings, all of it.
   **A changed line the author's report does not declare is a finding, whether
   or not it changes behaviour**, and the two cheapest places for one to hide
   are a fixture repaired beyond what the packet required and a comment updated
   in passing. This paragraph exists because exactly that slipped past two
   sealed reviewers on an earlier round of this change, one of whom was
   mutating the diff at the time.
   Then, for the behavioural subset: mutate lines you choose — at least one per
   changed function — run the affected tests, report the arm counts red and
   restored; a changed line with no arm is a finding; **a mutation that stays
   GREEN is the finding.** Restore by SHA-256. Re-run a sample of the author's
   named mutations rather than trusting the table, then at least one of your own
   against the weakest link. **Mutating tells you nothing about the lines you
   did not mutate, and the fixture case above is precisely those** — so
   enumerate first and mutate second, never the other way round.
...
## Output — write to `D:/addictedtoai-worktrees/fleet6-stage0-C1/REVIEW1.md`
