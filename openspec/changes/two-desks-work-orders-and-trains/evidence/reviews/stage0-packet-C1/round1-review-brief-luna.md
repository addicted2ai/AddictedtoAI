...
6. **THE DIFF FROM THE MERGE BASE IS THE LIST OF CHANGED BEHAVIOURAL LINES.**
   ...
   affected tests, report the arm counts red and restored; a changed line with
   no arm is a finding; **a mutation that stays GREEN is the finding.** Restore
   by SHA-256. Re-run a sample of the author's named mutations rather than
   trusting the table, then at least one of your own against the weakest link.
