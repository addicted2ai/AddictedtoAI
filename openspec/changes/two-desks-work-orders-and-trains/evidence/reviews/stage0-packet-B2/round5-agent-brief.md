# Packet B2 — tasks 10–12 of `two-desks-work-orders-and-trains` (author brief, round 5: the four narrower-than-property arms, finding 1 corrected)
...
- If a tool call is blocked or refused, record it in `RESULT5.md` and stop
...
Round 4 stopped, correctly, when its finding-1 arm went red against the
unmutated code. The architect has read the arm and the code and RULES: that
red is NOT a production defect. The separator test sets `maxChars` to
exactly the marker-only sum (chunk heading + the two cut markers + the
separators), and at that cap the excerpt renders the two CUT MARKERS and no
`### Requirement:` line — which is the design of the excerpt budget (task
6(d), packet B1: "what every named capability is guaranteed is its MARKER —
one line naming the requirement heading as cut — and nothing more"). The
arm asserted the `### Requirement:` heading set where the design guarantees
markers. Correct the ARM, not the code. The three other arms round 4 wrote
are in the worktree UNCOMMITTED (`git status` shows the two test files
modified); keep them, do not rewrite them, and take them through their named
mutations now.

All four findings are of ONE class: an arm that exists but is NARROWER than
the property it names — the named mutation stays green under it. For every
finding: run the named mutation against the arm, record it RED, restore,
record it GREEN. If an arm goes RED AGAINST THE CURRENT, UNMUTATED CODE and
the reason is not the one ruled above, STOP on that finding and report it in
`RESULT5.md` §7 with the output. NO further sweep this round.

### Finding 1 (REVIEW3 §1, corrected) — both cited headings from one constitution are present, `brief.test.mjs:236-258`
Named mutation: `loop/lib/specs.mjs:355-357` constitution candidates →
`.slice(0, 1)` stays green today. Replace the round-4 assertion at
`brief.test.mjs:254` with one that fits the cap the test sets: parse the cut
markers (`[... CUT: requirement "<heading>" from ...]`) in `excerpt.text`
and assert the sequence of quoted headings equals the two cited headings in
order — under the mutation only one marker is rendered, so it goes red. Add
a SECOND arm beside it at an ample cap (say `maxChars: 20000`, or
`BRIEF_EXCERPT_MAX_CHARS`) asserting the `### Requirement:` heading set of
the rendered excerpt equals the two cited headings in order (both sections
fit, so both render as sections); the same mutation must go red there too.
Keep the existing cap-invariant assertion (`cap=…; text.length=…;
chars=…`) as it is.

### Finding 2 (REVIEW3 §2) — the cap binds in heading mode, `brief.test.mjs:262-288`
Round 4's arm is in the tree. Named mutation: `loop/lib/specs.mjs:421-429`
`floorOverhead` → `0`; it must go red; restore.

### Finding 3 (REVIEW3 §3) — the revision brief's section ORDER, `review.test.mjs:364-392`
Round 4's arm is in the tree. Named mutation: `loop/lib/brief.mjs:798-812`,
the excerpts block moved before the diff block; it must go red; restore.

### Finding 4 (REVIEW3 §4) — `(preamble)` is not a heading, `specs.mjs:169`
Round 4's arm is in the tree (`review.test.mjs` "merge refuses `(preamble)`
cites on approve and revise"). Named mutation: the `(preamble)` exclusion
removed (`headings.add(section.heading)` unconditionally); it must go red;
restore.
...
- `RESULT5.md` — (new) your report, in the worktree root; see "## How to end"
...
## Mutations (perform, observe red, restore; table in RESULT5.md)
...
1. Read `REVIEW3.md`, then `RESULT4.md` (round 4 stopped on its stop rule; its uncommitted arms are in the tree), then the changed lines each finding
...
5. Commit ONLY the two test files (`git add` each by path; never
   `git add -A`); do not add `RESULT5.md` or `.agent-brief.md`.
...
Write `D:/addictedtoai-worktrees/fleet6-stage0-B2/RESULT5.md` — this exact
