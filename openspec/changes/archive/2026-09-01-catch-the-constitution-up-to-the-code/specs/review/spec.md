# review — delta for catch-the-constitution-up-to-the-code
...
The ADDED requirement covers two mechanisms at once, deliberately: the newer
`carry:` field and the older reviewer-noted proposal it was modelled on. The
older one was never specified either, and writing the new one into a
constitution that still did not mention its own precedent would leave the
smaller gap looking like a decision.

The two MODIFIED requirements are unchanged in substance. Each replaces an
opening paragraph that diagnosed the world as it stood *before* the requirement
landed, in the present tense, and therefore asserted a state of the system that
the requirement immediately below it abolishes.
