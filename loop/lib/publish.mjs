 ...
 */
export async function publishStep(ctx, { cfg, dryRun = false } = {}) {
  ...
  // The shared step's signature is `publishStep(root, { dryRun, log })`, with
  // `log` a logger object exposing `step(name, detail)`. Adapting to it here,
  // rather than asking it to adapt to the loop, is what keeps it one step.
  const log = { step: (name, detail) => ctx.log(`${name}${detail ? ' — ' + detail : ''}`) };
  const res = await fn(ctx.repoRoot, { dryRun, log });
  return {
