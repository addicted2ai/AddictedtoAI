...
/**
 * The recorded debt, counted — the standing exposure the frontier-metrics report
 * names beside its declared-metric count.
 *
 * WHY THIS JOIN EXISTS AT ALL. `separate-a-claim-from-a-fact` states "No index
 * value SHALL render on any surface until its metric is registered and its
 * republication decision records the right as cleared", with the scenario "An
 * unregistered index does not render". That gate was implemented on the surface
 * that reads the REGISTRY (`lib/render/frontier.mjs`), and it is true there. It
 * is not true of the corpus: model entries bind `benchmarks.artificial_analysis
 * .{intelligence,coding,agentic}_index` as `source: feed` facts and print those
 * numbers in prose, and Artificial Analysis's rights are unanswered
 * (addictedtoai-ego8, terms URL 404'd 2026-09-05).
 *
 * That exposure is measured and ratcheted already — by this module, which fails
 * on any NEW binding of a declined path and warns on the recorded ones, a list
 * with one legal direction — but nothing pointed the new requirement at it, so a
 * reader of the requirement was entitled to believe the site prints no
 * unregistered index value. The join is one sentence in a report rather than a
 * second gate, because a second gate over the same bindings would be the
 * duplicate the ratchet already is.
 *
 * @returns {{bindings: number, files: number, issue: string|null}}
 */
export function debtExposure(debt) {
  const known = debt?.known ?? [];
  return {
    bindings: known.length,
    files: new Set(known.map((e) => e?.file).filter(Boolean)).size,
    issue: typeof debt?.issue === 'string' ? debt.issue : null,
  };
}

/** `data/declined-binding-debt.json`, or an empty list when it is absent. */
export async function readDeclinedDebt(file = DECLINED_DEBT_FILE) {
  try {
    return JSON.parse(await readFile(file, 'utf8'));
  } catch (err) {
    // An absent file means an empty list — the same reading `declinedFieldsStep`
    // takes, and the safe direction: nothing forgiven, nothing to report.
