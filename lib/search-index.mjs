...
export function buildSearchIndex(corpus) {
   ...
   * right."* GREPPED before writing this, because each surface reaches a
   * document by a different route and only one of them is here.
   *
   * TASK 6 NAMED FOUR: the sitemap, the search index, llms.txt and the
   * INDEXABILITY JOIN. The grep found two the task did not name — the route
   * table and the Pulse's own corpus walk — and each is recorded below as its
   * own row rather than swapped in for one of the four, because "six surfaces
   * were in play, not four" is itself the finding, and a list that quietly
   * substitutes one for another hides it. The count is kept in step with the
   * tests deliberately: a record naming five while the suite pins six is a
   * record a reader has to re-derive, which is what a record is for.
   *
   ...
   *   indexability      `indexability(doc, indexOpts)` is called in exactly one
   *     join            place, inside `for (const doc of corpus.entry)`
   *                     (`lib/build-content.mjs:153`), and the review gate it
   *                     is fed from — `entryReviewGate` (`lib/reviews.mjs:561`)
   *                     — walks `corpus.entry` as well. A route-less type is out
   *                     by construction, and by more than an accident of which
   *                     list is walked: indexability is a question about a PAGE,
   *                     whether a crawler may index it, and a claim has no page
   *                     of its own to answer it about. Pinned by a test rather
   *                     than changed.
   *   route table       THE FIFTH, not named by task 6: `corpus.all` in
   *                     `lib/routes.mjs` — a claim would have added
   *                     `/wiki/org/x#claim-y` as a literal route the site is
   *                     expected to serve. FIXED there.
   *   the Pulse's       A SIXTH, and the same finding a second time: `readCorpus`
   *     corpus walk     (`pulse/lib/corpus.mjs`) enumerates its directories
   *                     explicitly and `content/claims` is not among them, and
   *                     the overdue-fact sweep (`pulse/lib/freshness.mjs`) walks
   *                     `corpus.entries`. So no re-check work is ever generated
   *                     for a claim — specs/wiki requires exactly that — but it
   *                     holds by construction, which is the kind of carve-out a
   *                     later edit to that directory list breaks in silence.
