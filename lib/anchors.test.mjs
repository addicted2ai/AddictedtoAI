...
// Read against a blog-less FIXTURE, not against the live `content/blog/`.
// This assertion originally ran on the real corpus and asserted it held zero
// posts — true on the day the blog was emptied (task 1.1) and false forever
// after the first post merges, which is the outcome the whole change exists to
// produce. A test that fails when the feature works is testing the calendar.
// The durable property is the one below: given a corpus with no posts, the step
// is clean and SAYS the count, so a check that ran on nothing cannot be mistaken
// for a check that ran on everything. The live corpus is covered by the build.
test('3.5 a corpus with zero posts is trivially clean, and says so', async () => {
  const res = await anchorCheckStep({
    contentRoot: fixtureRoot('facts'),
    changes: [],
    out: { write: () => {} },
  });
  assert.equal(res.posts, 0, 'the `facts` fixture carries no blog posts');
