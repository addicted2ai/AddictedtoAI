 ...
 * exactly backwards.
 *
 * The ceiling is therefore a HIGH-WATER MARK, and a high-water mark that is
 * never lowered lets the list regrow to it with the suite green. So the
 * discipline, stated here and in `data/declined-binding-debt.json`'s `note`:
 * whoever retires debt lowers this constant to the new `known` count IN THE
 * SAME DIFF. It is not lowered today because nothing has been repaid — the
 * debt file's `repaid` is empty, `known` is 48, and the ceiling is exact.
 * Paying it down is the content decision tracked as addictedtoai-226f.
 */
