...
test('11 claims order newest first by accessed', async () => {
  const { corpus, doc } = await subjectOf();
  const html = renderClaims(doc, { corpus });
  const order = [...html.matchAll(/id="claim-([a-z-]+)"/g)].map((m) => m[1]);
  assert.deepEqual(order, ['said-again', 'unlooked', 'looked-and-failed', 'confirmed', 'third-party']);
});
