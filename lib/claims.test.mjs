...
import { renderClaims, renderVerification, renderEntryPage } from './render/entry.mjs';
...
test('1 the fixture corpus loads six claim records through the real loader', async () => {
  const corpus = await claimsFixture();
  assert.equal(corpus.diags.errors.length, 0, corpus.diags.errors.map((e) => e.message).join('\n'));
  assert.equal(corpus.claim.length, 6);
  // TWO subjects, deliberately. With one, "renders the claims naming this
  // subject" and "renders every claim in the corpus" are the same output, and
  // the declared join in `renderClaims` cannot be falsified at all — see the
  // task-11 join test below, which is why the second entry exists.
  assert.equal(corpus.entry.filter((d) => d.data.kind === 'org').length, 2);
});
...
test('2 claimSchema is .strict(): a MISSPELLED optional key is refused, not silently dropped', async () => {
  // The mechanism task 2 names, measured rather than assumed. With `.strict()`
  // deleted, `validateFrontMatter('claim', ...)` returned ok=true for each of
  // these and dropped the key — and `verifed` is the one that matters most,
  // because `verified` is the ONLY optional key on this record, so its loss is
  // the silent one: a claim that WAS confirmed renders as absent, which this
  // record's whole tri-state exists to mean "nobody looked". The site would be
  // asserting silence about a check that happened.
  //
  // This repository's schema header names exactly this failure shape: "`alias:`
  // where `aliases:` was meant would otherwise parse cleanly into an entry with
  // no aliases and nothing downstream would ever notice."
  const valid = {
    subject: 'org/demo-vendor',
    field: 'agentic_task_completion',
    quote: 'Our model finishes the suite unattended.',
    source_url: 'https://demovendor.example/blog/a',
    source_host: 'demovendor.example',
    accessed: '2026-08-27',
  };
  assert.equal(validateFrontMatter('claim', valid).ok, true, 'the control: the record is valid');
  const confirmation = { by: 'a reviewer', url: 'https://e.example/x', date: '2026-08-28' };
  for (const [key, value] of [
    ['verifed', confirmation],
    ['verified_by', 'a reviewer'],
    ['domains', ['coding']],
  ]) {
    const res = validateFrontMatter('claim', { ...valid, [key]: value });
    assert.equal(res.ok, false, `${key} must be refused, not dropped`);
    assert.match(
      res.issues.map((i) => i.message).join('\n'),
      new RegExp(`unknown front-matter key\\(s\\): ${key}`),
      'and the message must name the key the author actually typed',
    );
  }
});
...
test('15 a publishes_from value that is a PUBLIC SUFFIX fails, and is not told to declare it', async () => {
  // `co.il` is a real two-label public suffix that MULTI_LABEL_SUFFIXES does
  // not list, so before this gate it validated as an ordinary registrable
  // domain and admitted every registrant under `.co.il` as one party.
  const err = await buildFixtureExpectingFailure('bad/publishes-from-suffix');
  assert.match(err.message, /wiki[/\\]org[/\\]demo-vendor\.md/, 'the entry');
  assert.match(err.message, /"co\.il"/, 'the value');
  assert.match(err.message, /PUBLIC SUFFIX/, 'what is wrong with it');
  assert.match(err.message, /MULTI_LABEL_SUFFIXES/, 'and where the real fix goes');
  assert.ok(
    !/declare "co\.il"/.test(err.message),
    'the old remediation message instructed the editor to declare the suffix — the build ' +
      'teaching the defect it is meant to catch',
  );
});

test('15 a publishes_from value that is a URL fails, and is NOT handed a reduction', async () => {
  // Measured before this clause existed: `https://kimi.ai` and five other
  // non-host shapes returned {ok: true}, because each equals its own dot-split
  // reduction. The entry validated, published through review and read as
  // declared — and `isSubjectOwned('platform.kimi.ai', it)` was FALSE. FM-N6
  // with the gate green, on the one field a gate can say anything about.
  const err = await buildFixtureExpectingFailure('bad/publishes-from-url');
  assert.match(err.message, /wiki[/\\]org[/\\]demo-vendor\.md/, 'the entry');
  assert.match(err.message, /"https:\/\/demobrand\.example\/blog"/, 'the value');
  assert.match(err.message, /not a hostname/, 'what is wrong with it');
  assert.ok(
    !/declare "https/.test(err.message),
    'and never a reduction: `platform.kimi.ai/blog` reduced to `kimi.ai/blog`, so the build told ' +
      'an editor to declare a string no source_host can ever equal',
  );
});
...
test('6 ROUTE TABLE — an `all`-shaped corpus is REFUSED, not quietly walked', async () => {
  // The carve-out was written as `corpus.documents ?? corpus.all`, and that
  // fallback silently restored the exact leak the line was changed to close:
  // any caller holding a corpus without `documents` — an older fixture, a
  // hand-built object, a future loader path — got the pre-change behaviour and
  // `/wiki/org/x#claim-y` entered the route table with nothing failing.
  const corpus = await claimsFixture();
  const legacy = { all: corpus.all, byId: corpus.byId };
  await assert.rejects(
    () => buildRouteTable(legacy, { appDir: fixtureRoot('claims', 'no-app') }),
    /documents/,
    'a corpus that predates the `routed` flag is a caller to fix, not a case to serve',
  );
});
...
test('6 INDEXABILITY JOIN — a claim is never asked whether a crawler may index it', async () => {
  const corpus = await claimsFixture();
  // Task 6's fourth named surface, which had no pin. Two halves, both measured
  // rather than reasoned from: the join walks `corpus.entry`, and the loaded
  // corpus is one where that matters (there ARE claims to leave out).
  assert.ok(corpus.claim.length > 0, 'the assertion below must not pass on an empty set');
  for (const doc of corpus.claim) {
    assert.equal(doc.index, undefined, 'nothing computes an indexability for a claim');
  }
  // And the source: `indexability()` is called once, inside a loop over
  // `corpus.entry`, and `entryReviewGate` — the gate feeding it — walks the
  // same list. A route-less type is out by construction, because indexability
  // is a question about a PAGE and a claim has none of its own.
  const build = readFileSync(join(ROOT, 'lib', 'build-content.mjs'), 'utf8');
  const calls = [...build.matchAll(/\bindexability\(/g)];
  assert.equal(calls.length, 1, 'one call site, so one list decides');
  assert.match(
    build.slice(0, calls[0].index),
    /for \(const doc of corpus\.entry\) \{[\s\S]*$/,
    'and that call site is inside the loop over corpus.entry',
  );
  const reviews = readFileSync(join(ROOT, 'lib', 'reviews.mjs'), 'utf8');
  assert.match(
    reviews,
    /export function entryReviewGate\([\s\S]{0,600}for \(const doc of corpus\.entry\)/,
    'the gate it is fed from walks corpus.entry too',
  );
});
...
test('6 THE PULSE — a claim generates no re-check work and can go nowhere near freshness', () => {
  // specs/wiki: *"A claim SHALL be treated as `dated` by construction. No
  // re-check work SHALL ever be generated for a claim record, and no overdue
  // marker SHALL render on one."* That held by construction and had no pin,
  // which made it the one carve-out a future edit to the Pulse's directory list
  // could break in silence. Pinned on the two files that would have to change.
  const pulseCorpus = readFileSync(join(ROOT, 'pulse', 'lib', 'corpus.mjs'), 'utf8');
  assert.ok(
    !/content\s*\+\s*'\/claims'|\bclaims\b/.test(pulseCorpus.replace(/\/\*[\s\S]*?\*\//g, '')),
    'readCorpus enumerates its directories explicitly (wiki, tutorials, directory/tools, learn, ' +
      'blog, deltas) and must never add claims: what ages on a claim is its VERIFICATION, on the ' +
      'verifier\'s clock, and "Anthropic said this on 2026-08-27" is true forever',
  );
  const freshness = readFileSync(join(ROOT, 'pulse', 'lib', 'freshness.mjs'), 'utf8');
  assert.ok(
    /for \(const entry of corpus\.entries\)/.test(freshness),
    'the overdue-fact sweep walks corpus.entries, so a claim has no path into it',
  );
  assert.ok(!/\bclaim\b/i.test(freshness.replace(/\/\*[\s\S]*?\*\//g, '')));
});
...
test('8 two claim files with the same BASENAME collide on the fragment and the review name', async () => {
  // A claim mints no URL, so — alone among the seven types — its slug is not
  // disambiguated by anything. The four-part duplicate check cannot see this:
  // these two records differ in field, source_url AND accessed, and are a
  // legitimate pair of claims in every respect except that they are both called
  // `speed.md`. What they collide on is `#claim-speed` (two DOM ids on one
  // page, and one `doc.url` for two records) and `seed-claim-speed.md` (two
  // records contending for one verdict).
  const err = await buildFixtureExpectingFailure('bad/claim-slug-collision');
  assert.match(err.message, /claims[/\\]first[/\\]speed\.md|claims[/\\]second[/\\]speed\.md/);
  assert.match(err.message, /duplicate claim slug "speed"/);
  assert.match(err.message, /#claim-speed/, 'the fragment it collides on');
  assert.match(err.message, /seed-claim-speed\.md/, 'and the review record it collides on');
});

test('8 the slug check is what catches the collision — the four-part check cannot', async () => {
  // MEASURED, so the test above cannot be passing on the wrong error. The two
  // colliding records agree on `subject` alone; the duplicate-claim rule needs
  // all four, so it files nothing and only the slug rule fires.
  const corpus = await loadCorpus({ contentRoot: fixtureRoot('bad', 'claim-slug-collision') });
  const rules = corpus.diags.errors.map((e) => e.rule);
  assert.deepEqual(rules, ['duplicate-claim-slug']);
});

test('9 claims are appended to reviewablePieces, after every prose surface, without moving anything else', async () => {
  const corpus = await claimsFixture();
  const pieces = reviewablePieces(corpus);
  // Body-less entries now follow the claims, appended for the same reason and
  // in the same position (specs/review, addictedtoai-kpgn,
  // say-what-a-review-record-covers): the order is part of the join, so a piece
  // type joins at the END or it moves which piece claims an ambiguously-named
  // record. What this test measures is unchanged — the claims sit after every
  // prose surface, and nothing ahead of them moved.
  const bodyless = corpus.entry.filter((d) => !d.hasBody);
  const claimsAt = pieces.length - bodyless.length - corpus.claim.length;
  assert.deepEqual(
    pieces.slice(claimsAt, claimsAt + corpus.claim.length).map((d) => d.type),
    corpus.claim.map(() => 'claim'),
    'appended after the prose surfaces: an insertion anywhere earlier would move which piece claims an ambiguously-named record',
  );
  assert.deepEqual(
    pieces.slice(claimsAt + corpus.claim.length).map((d) => d.file),
    bodyless.map((d) => d.file),
    'and only the body-less entries follow them',
  );
  assert.deepEqual(
    pieces.slice(0, corpus.entry.filter((d) => d.hasBody).length).map((d) => d.file),
    corpus.entry.filter((d) => d.hasBody).map((d) => d.file),
    'the bodied entries are still first and unmoved',
  );
});

test('10 neither `verified` nor `claims` is a MECHANICAL_FRONT_MATTER_KEY', () => {
  // NON-MEMBERSHIP, NOT EXACT LIST EQUALITY, and the difference is the whole
  // point of this note. This test first asserted the list was exactly
  // ['timeline']. That was a guardrail keyed on the list's LENGTH standing in
  // for a check on its CONTENT: the concurrent change `tag-the-corpus-by-domain`
  // added `domains_seeded` — a licensed, reviewed addition this change has no
  // opinion about — and the assertion went red on correct code, on a sibling
  // change, with the noise landing on people who had not touched claims. A
  // check that fires on its own machinery is how a guardrail gets switched off,
  // which lib/review-hash.mjs's own header says in as many words.
  //
  // The EXACT-CONTENTS pin still exists and belongs where the list does:
  // lib/review-hash.test.mjs 'zlq (a)', maintained by whoever changes the list.
  // Task 10 asks only what is asserted here, and asks it for a reason no length
  // check carries.
  assert.ok(
    !MECHANICAL_FRONT_MATTER_KEYS.includes('verified'),
    'the exemption list is matched by key NAME across every content kind (lib/review-hash.mjs:99-102), ' +
      ...
      'is a judgment, which publishes through review',
  );
  assert.ok(
    !MECHANICAL_FRONT_MATTER_KEYS.includes('claims'),
    '`claims` is absent for the same reason and because there is no `claims` key: the record sits ' +
      'BESIDE the entry, not on it',
  );
  // Positive control: without this the two assertions above would both pass on
  // an empty list, i.e. on a list that had stopped exempting anything at all.
  assert.ok(
    MECHANICAL_FRONT_MATTER_KEYS.includes('timeline'),
    'the list is non-empty and still exempts the Pulse-maintained key it was created for',
  );
});
...
test('11 the subject join is DECLARED — a record filed against another party renders on ITS page only', async () => {
  // THE FILTER IS THE JOIN, and with one subject in the fixture it could not be
  // falsified: "renders the claims naming this subject" and "renders every claim
  // in the corpus" were the same output. Deleting the filter passed the whole
  // file. Two subjects is what makes it a test.
  //
  // What a missing filter ships is implementer-ledger rows 2 and 4's class
  // arriving through the repair: a record filed against org/A rendered on every
  // entry page in the site, under a heading saying that party said it. The
  // delta: the join is *"declared, never inferred: no name match, no host match,
  // no title match."*
  const { corpus, doc } = await subjectOf();
  const other = corpus.byId.get('org/other-party');
  assert.ok(other, 'the fixture must actually contain a second subject');

  const mine = renderClaims(doc, { corpus });
  assert.deepEqual(
    slugsIn(mine).sort(),
    ['confirmed', 'looked-and-failed', 'said-again', 'third-party', 'unlooked'],
    'the subject entry renders its own five records and no more',
  );
  assert.ok(
    !mine.includes('filed-against-the-other-party'),
    'a claim naming org/other-party is not this party\'s to answer for',
  );
  assert.ok(!mine.includes('Other Party'), 'and the other party is not named on this page at all');

  const theirs = renderClaims(other, { corpus });
  assert.deepEqual(
    slugsIn(theirs),
    ['filed-against-the-other-party'],
    'and the other subject renders exactly its own',
  );
  assert.ok(!theirs.includes('claim-unlooked'));
});
...
/* ------------------------------------------------------------------ *
 * THE WHOLE PIPELINE, ONCE, WITH THE ROUTE-LESS TYPE PRESENT
 *
 * Every test above calls `loadCorpus()` or a renderer directly, and the five
 * carve-out tests each check ONE surface in isolation. The change's own stated
 * risk is *"the carve-outs a route-less content type needs … the largest unknown
 * in the change"*, and no assertion above observes the route table, the search
 * index, backlinks, the origin scan, the currency scan and the review join
 * surviving a seventh type in ONE pass. `buildFixtureExpectingFailure` runs the
 * real build five times for the bad fixtures; nothing ran it for the good one.
 * ------------------------------------------------------------------ */

test('1-16 the REAL build runs over a corpus containing claim records, and the page carries them', async () => {
  const site = await buildFixture('claims');
  assert.equal(site.corpus.claim.length, 6);
  assert.equal(site.corpus.documents.length, 2, 'two org entries, and no claim among the documents');

  const html = renderEntryPage(site.corpus.byId.get('org/demo-vendor'), site);
  // From the real pipeline, not from a hand-assembled {corpus} object: the
  // vendor list, the third-party list, and the fragment a claim's URL points at.
  assert.match(html, /id="claim-unlooked"/);
  assert.match(html, /What this party says about itself/);
  assert.match(html, /What other parties say about it/);
  assert.match(html, /openrouter\.ai/, 'the third party is attributed to its own domain');
  // and the declared join holds through the build too
  assert.ok(!html.includes('filed-against-the-other-party'));
});
