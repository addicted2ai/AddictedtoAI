...
// ── 2yb: the masks must not cascade ──────────────────────────────────────
//
// Each mask is matched against the ORIGINAL text. When they cascaded, the
// inline-code mask ran first, so a line opening with a code span BEGAN with
// blanks and the indented-code pattern (/^ {4,}\S.*$/gm) then ate the rest of
// it. That is the single most common sentence shape in this corpus, and the
// check silently found nothing in it.

test('2yb a leading inline code span does not blank the rest of its line', () => {
  const body = '`openai/gpt-5`, listed in March, costs $3.00 per million tokens.\n';
  const hits = findCurrencyLiterals(body);
  assert.equal(hits.length, 1, 'the $3.00 after a leading code span is still found');
  assert.match(hits[0].text, /\$3\.00/);
});

test('2yb POSITIVE CONTROL — the same sentence without the code span behaves identically', () => {
  const body = 'openai/gpt-5, listed in March, costs $3.00 per million tokens.\n';
  const hits = findCurrencyLiterals(body);
  assert.equal(hits.length, 1, 'the control that always passed, kept so the pair discriminates');
  assert.match(hits[0].text, /\$3\.00/);
});

test('2yb what the masks are FOR still works — code and genuine indented blocks are ignored', () => {
  // A price inside an inline code span is not prose.
  assert.deepEqual(findCurrencyLiterals('the literal `$3.00` is shown as code\n'), []);
  // A genuinely indented code block is still masked, span or no span.
  assert.deepEqual(findCurrencyLiterals('text\n\n    const price = "$3.00";\n'), []);
});

test('2yb an astral character does not shift the mask off its target', () => {
  // `m.index` counts UTF-16 code units; a code-point spread would desynchronise
  // every mask after the first astral character. The emoji is two code units.
  const body = '\u{1F600} `openai/gpt-5` then $3.00 in prose\n';
  const hits = findCurrencyLiterals(body);
