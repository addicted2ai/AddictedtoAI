...
/**
 * The member-max dates for the sitemap's index routes. The resolver functions
 * are passed in already bound to the site's review and changed-feed data, so
 * this is the single definition of which members each index summarizes.
 * Signature: `indexRouteDates(site, contentChangedOn, postChangedOn, changedOn)`.
 * `postChangedOn` is the third parameter and `changedOn` the fourth; the
 * three-argument form sketched in addictedtoai-nq36 omitted the post-specific
 * resolver, but the blog fold needs it to preserve publication-date fallback
 * and edit-date behaviour.
 */
export function indexRouteDates(site, contentChangedOn, postChangedOn, changedOn) {
  const wiki = newest(site.browsable.map((doc) => contentChangedOn(doc)));
  const learn = newest(site.corpus.learn.map((doc) => contentChangedOn(doc)));
  const tools = newest(site.tools.map(({ doc }) => doc.data.last_verified));
  const tutorials = newest(
    site.tutorials.filter(({ state }) => state.indexed).map(({ doc }) => doc.data.verified_on),
  );
  const blog = newest(site.posts.map((doc) => postChangedOn(doc)));
  const deltas = newest(site.deltas.map((view) => contentChangedOn(view.doc)));
  const catalog = newest(
    site.catalog.map((row) => (row.entry ? changedOn.get(row.entry.id) : undefined)),
  );
  const deprecations = newest(
    site.deprecations.map((row) => (row.entry ? changedOn.get(row.entry.id) : undefined)),
  );
  const feed = newest(site.changes.map((line) => line.date));
  const data = newest([wiki, catalog, deprecations, deltas, feed, blog, tutorials]);

  return { wiki, learn, tools, tutorials, blog, deltas, catalog, deprecations, feed, data };
}
