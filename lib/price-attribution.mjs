 ...
 *  - THE BOUNDARY MUST BE QUOTE-AWARE. This corpus writes `...before 2026."`
 *    with the full stop INSIDE the closing quote, and a bare `[.!?](?=\s|$)`
 *    does not split there because the next character is `"` and not
 *    whitespace, so two sentences read as one and the second inherits the
 *    first's verb. Measured in `arxiv-pin.mjs`, which hit it for real and
 *    filed it here as **addictedtoai-q3n**; latent here only because prices
 *    are not written inside quotation marks today. The class carries `)` and
 *    `]` as well as the quote marks: a price sentence in this corpus ends
 *    `... (top provider's rate).` — closers may also FOLLOW the stop.
 */
