 ...
 *     `lib/changes.mjs:111-119` keys one Map on `${source}|${rowId}`, so of two
...
export function renderLeadChangeStrip(changeLines) {
   ...
   */
  //
  // THE KIND IS PART OF THE FILTER, and it was not. The producer is careful
  // about the distinction — the seeder writes the baseline as an `annotation`
  // with no `annotates`, and its comment says in as many words that it is
  // emphatically NOT a `lead-change` line — but the reader did not enforce what
  // the writer was careful about, so the invariant lived in one half of a pair.
  // An unscoped filter renders ANY line carrying a truthy `baseline` as "The
  // record of X begins …", a `lead-change` line included, which would then
  // appear twice: once as an event in the strip above and once as the sentence
  // saying the record starts there. That is the two sentences specs/pulse is
  // precise about keeping apart.
  const baselines = lines
    .filter((l) => l && l.kind === KIND.ANNOTATION && l.baseline && l.date)
