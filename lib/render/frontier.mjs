 ...
 *
 * The regex is replaced by an EXPLICIT ALLOW-LIST in two ranked tiers, taken
...
const CLAIM_FIELDS_QUANTIFIED = new Set([
  'cost_per_task',
  'fast_mode_speed',
  'observed_latency_p50',
  'observed_throughput_p50',
  'output_tokens_per_task',
]);

/** tier 1 outranks tier 2; 0 = not a vendor claim this column may carry. */
function claimRank(fact) {
  ...
  if (CLAIM_FIELDS_BENCHMARK.has(field)) return 2;
  if (CLAIM_FIELDS_QUANTIFIED.has(field)) return 1;
  return 0;
}

/**
 * The model's own cited, quantified performance claim, or null. Never reads
 * an org (RD-002 fix 1 stands). Ranked, not first-match (RD-003 fix 1).
 */
function vendorClaimFact(modelDoc) {
  let best = null;
  let bestRank = 0;
  for (const f of modelDoc?.data?.facts ?? []) {
    const rank = claimRank(f);
    if (rank > bestRank) {
      best = f;
      bestRank = rank;
    }
