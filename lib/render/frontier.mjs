 ...
 * domains an organisation is recorded citing itself from, and an org's OWN
 * domain among them is one whose host carries the org's own name — the same
 * alias vocabulary `matchProviders` already joins on. A host is the vendor's
 * when it is one of those recorded domains (or a subdomain of one), or when
 * one of its labels IS one of the org's own name tokens — so
 * `www.anthropic.com` and `blog.google` (recorded) and `deepmind.google` (not
 * recorded, but named) are Google DeepMind's and Anthropic's own, while
 * `openrouter.ai`, `llm-releases.com`, `huggingface.co`, `venturebeat.com`,
 * `en.wikipedia.org` and every other third party are not — whoever republishes
 * a vendor's number, the number they publish is theirs, not the vendor's.
 ...
 * `labs.<anything>`.
 *
 * Attribution, JV-sys F-sys-4-1: the cell now names the VENDOR as the claim's
 * source — "<claim fragment> — Google DeepMind, accessed <date>" — so the READ
...
/** Does any label of `host` name the organisation? */
function hostNamesOrg(host, tokens) {
  return host.split('.').some((label) => label.length >= 3 && tokens.has(label));
}

/**
 * The domains an ORG ENTRY records as its OWN — the hosts it cites itself
 * from, kept only where the host carries the org's own name, so a wikipedia
 * or feed citation in the same list is not mistaken for the vendor's site.
 */
function orgOwnDomains(org) {
  ...
  for (const item of cited) {
    const host = urlHost(item?.source_url);
    if (host && hostNamesOrg(host, tokens)) domains.add(host);
  }
  return domains;
}
...
function isVendorSourced(fact, org) {
  const host = urlHost(fact?.source_url);
  if (!host || !org) return false;
  for (const d of orgOwnDomains(org)) if (host === d || host.endsWith(`.${d}`)) return true;
  return hostNamesOrg(host, orgNameTokens(org));
}
    ...
    claimFact
      ? el(
          ...
          el(
            ...
            ' — ',
            // RD-004 / JV-sys F-sys-4-1: the ATTRIBUTION is the vendor's own
            // name, not "model record". The READ cell one column over states
            // the feed the price and context values were read from, the same
            // value on every row; with the claim's own source unnamed, the
            // row read as though that feed had sourced the vendor's claim.
            // The name is the link to the cited page, so the provenance the
            // cell carried is unchanged — it is now attributed in words.
