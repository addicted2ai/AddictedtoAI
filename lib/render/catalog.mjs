...
/**
 * Rows grouped into native `<details>` per provider (R-D / I14: the 396-row
 * table at 390px measured 93,963px stacked flat, one long scroll with no
 * addressable structure). Reuses `renderCatalogTable`'s own row data — the
 * desktop table is unchanged and stays the canonical view at >=34rem; this
 * is a second, narrow-only presentation of the SAME rows, closed by default
 * so the page's total scroll height at rest is providers-count tall, not
 * rows-count tall. `<details>` is native: no script opens or closes it, and
 * Chromium/Firefox both expand a closed `<details>` automatically when a
 * page-find (Ctrl-F) matches text inside it, which is what makes every row
 * "reachable without a route change" true for this closed-by-default form
 * (I14's own requirement) rather than only for an always-open one.
 *
 * Declined, with cause: wiring `CatalogFilter` (data-provider/-status/-name/
 * -price-in) to these `<li>` rows too, so the filter form affects this view
 * as well as the hidden desktop table. The packet's fence states
 * "CatalogFilter unchanged"; extending its selectors to a second row shape
 * is a behaviour change to that component this build does not make. Typing
 * into the filter at <=390px currently has no visible effect — a known
 * limitation, not a silent one — because the desktop table it operates on
 * is hidden at that width, not because filtering was attempted and failed.
 */
export function renderCatalogGroups(rows) {
  const groups = new Map();
  for (const r of rows) {
    const key = r.provider || 'unlisted';
    if (!groups.has(key)) groups.set(key, []);
    groups.get(key).push(r);
  }
  return el(
    'div',
    { class: 'catalog-groups', 'data-catalog-groups': String(groups.size) },
    [...groups.entries()]
      .map(([provider, list]) =>
        el(
          'details',
          { class: 'provider-group', 'data-provider-group': provider },
          el(
            'summary',
            {},
            el('span', { class: 'provider-name' }, escapeHtml(provider)),
            el('span', { class: 'provider-count' }, ` (${list.length})`),
          ),
          el(
            'ul',
            { class: 'provider-rows' },
            list.map((r) => groupRow(r)).join(''),
          ),
        ),
      )
      .join(''),
...
function groupRow(row) {
  return el(
    'li',
    { class: 'provider-row', 'data-status': row.status ?? '' },
    row.entry
      ? link(row.entry.url, row.name, { class: 'provider-row-name' })
      : el('span', { class: 'provider-row-name' }, escapeHtml(row.name)),
    el(
      'span',
      { class: 'provider-row-meta' },
      el('span', { class: 'provider-row-price' }, row.price_input ? `in ${escapeHtml(row.price_input)}` : 'in —'),
      el('span', { class: 'provider-row-price' }, row.price_output ? `out ${escapeHtml(row.price_output)}` : 'out —'),
      row.status ? badge(row.status, statusTone(row.status)) : '',
    ),
  );
}
