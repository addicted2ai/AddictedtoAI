...
test('the strip states where the record begins, and never that anything led there', () => {
  // The second half of the SHALL, which had no home: "Its limits SHALL be stated
  // on the surface rather than implied: the record begins when observation
  // began, and a baseline line says *observation began here*, not *this model
  // became the leader here*." The per-line seeded marker satisfies "each marked
  // as seeded" and nothing else — a reader seeing seeded lines could not tell
  // whether the record starts last week or at the dawn of time.
  const baseline = {
    key: 'seed|frontier|fixture-index|baseline',
    date: '2026-09-01',
    kind: 'annotation',
    seeded: true,
    baseline: true,
  ...
  const html = renderLeadChangeStrip([{ ...LEAD_LINE, seeded: true }, baseline]);
  assert.match(html, /The record of Fixture Index begins/);
  assert.match(html, /2026-09-01/, 'the earliest committed snapshot date, read from the baseline line');
  assert.match(html, /not where anything took the lead/, 'observation began here, not "this became the leader here"');
  assert.match(html, /data-derived="frontier-baseline"/, 'the date sits inside a data-derived fence');
  assert.equal(/lead changed on Fixture Index/.test(html.split('strip-baseline')[1] ?? ''), false, 'the baseline is not an event');

  // READ, NOT RESTATED. With no baseline written the sentence does not appear —
  // so it is the backfill's artefact reaching the surface rather than a constant
  // in the template that would say "the record begins" whatever the data holds.
  assert.equal(/The record of/.test(renderLeadChangeStrip([LEAD_LINE])), false);
  assert.equal(/The record of/.test(renderLeadChangeStrip([])), false);

  // And it renders on a day with no lead change at all, which is the day the
  // statement matters most.
  assert.match(renderLeadChangeStrip([baseline]), /No lead change recorded yet[\s\S]*The record of Fixture Index begins/);

  // THE FORBIDDEN HALF, which is the half the reader was missing. The seeder is
  // careful that a baseline is an `annotation` and emphatically not a
  // `lead-change`; the filter here was not scoped by kind at all, so any line
  // carrying a truthy `baseline` rendered the sentence — a `lead-change` line
  // included, which would then appear BOTH as an event in the strip above and
  // as the sentence saying the record starts there. An invariant enforced only
  // by its writer is an invariant one careless producer removes.
  const mislabelled = { ...LEAD_LINE, baseline: true, metric_label: 'Fixture Index' };
  const wrong = renderLeadChangeStrip([mislabelled]);
  assert.equal(
    /The record of/.test(wrong),
    false,
    'a lead-change line carrying `baseline: true` renders no baseline paragraph — the kind is READ, not assumed',
  );
  assert.match(wrong, /lead changed on Fixture Index/, 'and it is still rendered as the event it declares itself to be');
});
