...
export function renderDelta(view, opts = {}) {
  // `??` would have turned an explicit `heading: null` back into 'h3', which
  // is exactly what `renderDeltaPage` passes to suppress the heading — so a
  // delta's own page rendered its title twice, once as the h1 and again as a
  // linked h3 immediately under it. The default applies only when the caller
  // said nothing.
  const heading = 'heading' in opts ? opts.heading : 'h3';
  return el(
    'article',
