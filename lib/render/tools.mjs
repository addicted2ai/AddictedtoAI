...
import { el, join, link, extLink, date, sortNote, escapeHtml, notice, modeOf } from './common.mjs';
 ...
 *
 * `opts.defaultVerified` — I36 (iter-08, R8's iter-07 addendum): the
 * category's own dominant `last_verified` date (computed once per category
 * by `categorySection`, below, the same shape `renderCatalogTable` already
 * uses for `defaultFetched`). A row whose date MATCHES it is the
 * unexceptional case and stays at .listing-line's own muted weight
 * (globals.css); a row whose date DIFFERS carries `data-default="no"`, which
 * globals.css raises to ink — the exception is the value actually worth a
 * reader's attention, mirroring the badge clause's own default/exception
 * split rather than inventing a parallel one. */
export function renderListingRow({ doc, state }, byId, opts = {}) {
  const entry = byId.get(doc.data.entry);
  const isDefaultVerified = state.last_verified != null && state.last_verified === opts.defaultVerified;
  return el(
    'li',
    ...
    // I11 (iter-07, R13): `.listing-line` used to wrap all three fields in one
    // run-on inline string, joined by ` · ` separators, with no shared column
    // across a category's entries — the same defect R13 already fixed on
    // `.browse-row`. Its three children are now three explicit grid items
    // (globals.css makes `.listing-line` `display: contents` so they align
    // as columns without changing this element's own semantic grouping) —
    // "verified " and its date are combined into ONE `.listing-verified` span
    // so they form a single grid cell rather than a bare text node the grid
    // cannot place. The middot separators are dropped: column position is
    // now what separates the fields, the way it already does on `.browse-row`.
    el(
      'p',
      { class: 'listing-line' },
      ...
      el(
        'span',
        { class: 'listing-verified', 'data-default': isDefaultVerified ? 'yes' : 'no' },
        'verified ',
        date(state.last_verified),
      ),
      entry ? link(entry.url, entry.data.display_name, { class: 'listing-entry' }) : '',
    ),
    marker(state),
  );
}
...
export function categorySection(group, byId) {
  // I36 (iter-08): the category's own dominant verification date — see
  // renderListingRow's own note on `opts.defaultVerified` above. Scoped per
  // category, not per page, because .listings is one grid per category
  // (S19's own scope) and that is the column a reader actually reads down.
  const defaultVerified = modeOf(group.listings.map((t) => t.state.last_verified));
  return el(
    'section',
    ...
    el(
      'ul',
      { class: 'listings' },
      group.listings.map((t) => renderListingRow(t, byId, { defaultVerified })).join(''),
    ),
  );
}
