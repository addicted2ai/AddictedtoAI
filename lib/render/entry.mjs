 ...
 * The whole page body.
 *
 * I40 (iter-09, R13's iter-07(a) addendum): FACTS, TIMELINE and RAILS are
 * grouped inside one `.entry-side` wrapper rather than rendered as three
 * separate `<article>` children. This is a PRESENTATION change only — the
 * markup order within the wrapper is unchanged (facts, timeline, rails), so
 * a screen reader encounters the identical document either way, matching
 * this file's own long-standing rule for `order`-based reflow above. See
 * globals.css's own note on `.entry-side` for why a wrapper, not three
 * independently-placed grid items, is what actually closes the dead-track
 * gap between FACTS and RAILS on the two-column template.
 *
