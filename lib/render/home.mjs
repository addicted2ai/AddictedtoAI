    ...
    rows
      .map(({ org, row, modelDoc }) =>
        el(
          ...
          link(org.url, org.data.display_name, { class: 'frontier-door-org' }),
          row
            ? (modelDoc ? link(modelDoc.url, row.display_name ?? row.row_id, { class: 'frontier-door-model' })
                : el('span', { class: 'frontier-door-model' }, escapeHtml(row.display_name ?? row.row_id)))
            : el('span', { class: 'frontier-door-model absent' }, 'no listing in feed'),
        ),
      )
      .join(''),
  );
