...
import { reviewedHash, reviewedHashOfFile } from './review-hash.mjs';
...
import { hasProseBody, requiredRecordPieces } from '../scripts/verify-launch.mjs';
...
test('15ar a record that TIES on one piece still binds the pieces it alone reviewed', async () => {
  // THE DEFECT (addictedtoai-15ar), the third site of the shape addictedtoai-sge
  // and addictedtoai-9ls8 each cost a verify-launch failure. `unorderable` was
  // a Set of RECORD NAMES, so a record that could not be ordered against a
  // rival on ONE piece was refused on EVERY piece it declared in `subject:` —
  // including pieces where it was the only review that had ever existed. Those
  // pieces then read `missing` (unreviewed) and failed verify-launch, though a
  // matching approval sat right there in the records directory.
  //
  // The tie is the reachable trigger the bead names: two hand-written records
  // carrying neither `date:` nor a `j-` job id, both declaring one piece.
  const dir = mkdtempSync(join(tmpdir(), 'atai-reviews-partial-tie-'));

  // One record reviews two pieces; nothing orders it.
  writeRecord(dir, 'seed-two-pieces.md', {
    cite: 'one review, two pages',
    subject: [fileOf('mismatched'), fileOf('unbound')],
    reviewed: {
      [fileOf('mismatched')]: hashOf('mismatched'),
      [fileOf('unbound')]: hashOf('unbound'),
    },
  });
  // A rival on the FIRST piece only, equally unorderable.
  writeRecord(dir, 'seed-rival.md', {
    cite: 'the rival review',
    subject: fileOf('mismatched'),
    reviewed: { [fileOf('mismatched')]: hashOf('mismatched') },
  });

  const corpus = await loadCorpus({ contentRoot: CORPUS });
  const j = reviewJoin(corpus, { reviewsDir: dir });

  // The tie itself is unchanged: the contended piece binds neither record and
  // is reported once, because a tie-break invented here is a guess.
  assert.equal(j.byFile.has(fileOf('mismatched')), false);
  const c = j.contended.find((x) => x.includes('mismatched.md'));
  assert.ok(c, `the tie is reported: ${j.contended.join(' | ')}`);

  // THE ASSERTION THIS TEST EXISTS FOR: the OTHER piece — which only
  // `seed-two-pieces.md` ever reviewed, and where there is nothing to tie
  // with — still binds it.
  const other = j.byFile.get(fileOf('unbound'));
  assert.ok(other, 'a tie on one piece must not disown the record on another');
  assert.equal(other.record.name, 'seed-two-pieces.md');
  assert.notEqual(
    j.states.get(fileOf('unbound')),
    'missing',
    'a piece reviewed and approved must never report as unreviewed',
...
test('15ar a tie on one piece does not disown the FILENAME record of another', async () => {
  // THE SECOND SITE the bead names, and the one the first pass left standing:
  // "addDeclared refuses a record on EVERY piece it names if unorderable.has
  // (rec.name), AND taken() does the same for filename-derived candidates."
  // Converting only `addDeclared` fixed the front-matter half; `add()` still
  // asked `taken(rec.name)`, which reads `unorderable` by NAME. So a record
  // that ties on a piece it declared in `subject:` was still refused on the
  // piece whose CANONICAL RECORD NAME it is — a piece it alone ever reviewed,
  // reached by filename and not by front matter at all.
  //
  // The distinction from the test above is the whole point: there the record
  // reached the second piece through `subject:` (addDeclared); here it reaches
  // it through `reviewCandidates()` (add). One code path was fixed, the other
  // was not, and only a test entering by the second path can tell them apart.
  const dir = mkdtempSync(join(tmpdir(), 'atai-reviews-filename-tie-'));

  // The canonical record for `unbound` BY FILENAME — carrying that piece's
  // current hash — which also declares a DIFFERENT piece in `subject:`.
  writeRecord(dir, 'seed-wiki-concept-unbound.md', {
    cite: 'the only review unbound has ever had',
    subject: fileOf('mismatched'),
    reviewed: { [fileOf('unbound')]: hashOf('unbound') },
  });
  // A rival on `mismatched` only, equally unorderable: neither record carries
  // a `date:` or a `j-` job id, so the join can order neither.
  writeRecord(dir, 'seed-rival.md', {
    cite: 'the rival review',
    subject: fileOf('mismatched'),
    reviewed: { [fileOf('mismatched')]: hashOf('mismatched') },
  ...
  // The tie is unchanged: the contended piece binds neither and is reported.
  assert.equal(j.byFile.has(fileOf('mismatched')), false);
  assert.ok(
    j.contended.find((x) => x.includes('mismatched.md')),
    `the tie is reported: ${j.contended.join(' | ')}`,
  );

  // THE ASSERTION THIS TEST EXISTS FOR: the piece this record is named after
  // still binds it, and reads its recorded hash rather than `missing`.
  const own = j.byFile.get(fileOf('unbound'));
  assert.ok(own, 'a tie on a declared piece must not disown the record on the piece it is named for');
  assert.equal(own.record.name, 'seed-wiki-concept-unbound.md');
  assert.equal(
    j.states.get(fileOf('unbound')),
    'recorded',
    'a piece reviewed and approved must never report as unreviewed',
...
/* ===========================================================================
 * say-what-a-review-record-covers — THE BINDING COVERS BODY-LESS ENTRIES
 * (specs/review, beads addictedtoai-kpgn; N5 and N6).
 *
 * A body-less entry carries its sourced facts in front matter, which is INSIDE
 * the reviewed surface — so a fact-only edit to one moves that surface exactly
 * as a prose edit moves a page's. Filtered out of the reviewable set, that class
 * of edit produced no signal at any layer: not missing, not unbound, not
 * mismatched, absent from the count entirely. Two fixtures carry the two halves:
 * `concept/stub-bound` (a record binds it) and `concept/stub-missing` (none
 * does), both with a `facts:` block and no prose body at all.
 * ======================================================================== */

test('kpgn (N5) a fact-only edit to a BOUND body-less entry reports mismatched', async () => {
  const corpus = await loadCorpus({ contentRoot: CORPUS });
  const stub = corpus.entry.find((d) => d.data.id === 'concept/stub-bound');
  assert.ok(stub, 'the body-less fixture must load');
  assert.equal(stub.hasBody, false, 'and it must really have no prose body');

  // The record bound the surface as it stood BEFORE the fact moved. Computed by
  // hashing the file's own text with one fact VALUE changed — not by a made-up
  // constant — so this measures the reviewed surface reacting to a fact edit
  // rather than measuring that two strings differ.
  const abs = join(CORPUS, 'wiki', 'concept', 'stub-bound.md');
  const asReviewed = readFileSync(abs, 'utf8').replace('128,000 tokens', '64,000 tokens');
  assert.notEqual(asReviewed, readFileSync(abs, 'utf8'), 'the fixture must actually carry that fact');
  assert.notEqual(reviewedHash(asReviewed), hashOf('stub-bound'), 'and a fact edit must move the surface');

  const dir = mkdtempSync(join(tmpdir(), 'atai-reviews-stub-'));
  writeRecord(dir, 'seed-wiki-concept-stub-bound.md', {
    cite: 'the stub review',
    date: '2026-09-01',
    job: 'j-20260901-01',
    reviewed: { [fileOf('stub-bound')]: reviewedHash(asReviewed) },
  });

  const j = reviewJoin(corpus, { reviewsDir: dir });
  assert.equal(j.states.get(fileOf('stub-bound')), 'mismatched');
  const report = reviewStateReport(j);
  const problems = mismatchProblems(report);
  assert.equal(problems.length, 1, problems.join(' | '));
  assert.match(problems[0], /stub-bound\.md/);
  assert.match(problems[0], /REVIEWED THEN CHANGED/);

  // The other half: a body-less entry with no record is `missing`, and `missing`
  // produces no problem string — extending the binding is not extending the
  // obligation to have a record.
  assert.equal(j.states.get(fileOf('stub-missing')), 'missing');
  assert.ok(!problems.join('\n').includes('stub-missing'));

  // The counts move by exactly the body-less entries, so the report's `total`
  // is asserted rather than only its `mismatched` list: a set that grew by the
  // wrong pieces would pass every assertion above.
  assert.equal(report.total, reviewablePieces(corpus).length);
  assert.equal(
    report.total,
    corpus.entry.length + corpus.learn.length + corpus.tutorial.length +
      corpus.post.length + corpus.delta.length + (corpus.claim ?? []).length,
    'every entry is in the set now, bodied or not',
  );
});

test('kpgn (N6) the launch check\'s required-record list keeps its own prose bar', async () => {
  // THE BOUNDARY, on the real corpus and not a fixture. An implementer who wires
  // the extended set into `verify-launch`'s required list gets this one red test
  // instead of one launch failure per body-less entry.
  //
  // BOTH SIDES COMPUTED, NEVER LITERAL. The corpus was 96 bodied and 457
  // body-less when this was written; a literal here turns red on the next entry
  // the Pulse mints, which is exactly the failure the fixture-clock convention
  // exists to prevent.
  const corpus = await loadCorpus({});
  const required = requiredRecordPieces(corpus);
  const requiredEntries = required.filter((d) => d.type === 'entry');

  // The two lists are measured with their OWN predicates: `hasProseBody` is a
  // word count in verify-launch, `hasBody` is the corpus's own flag, and they
  // are not the same test.
  assert.deepEqual(
    requiredEntries.map((d) => d.file).sort(),
    corpus.entry.filter(hasProseBody).map((d) => d.file).sort(),
  );
  const bodyless = corpus.entry.filter((d) => !d.hasBody);
  assert.ok(bodyless.length > 0, 'the corpus must hold body-less entries, or this measures nothing');
  const requiredFiles = new Set(required.map((d) => d.file));
  for (const d of bodyless) {
    assert.ok(!requiredFiles.has(d.file), `${d.file} must not be required to carry a record`);
  }

  // And the reviewable set grew by exactly those entries — no more, no fewer.
  ...
  assert.equal(pieces.length, withoutStubs.length + bodyless.length);
  // Order is part of the join: the bodied entries stay first and unmoved, so no
  // ambiguously-named record changes hands for content nobody touched.
  assert.deepEqual(
    pieces.slice(0, withoutStubs.length).map((d) => d.file),
    withoutStubs.map((d) => d.file),
  );

  // Tool listings stay OUT — "every wiki entry", not "every content file a
  // record can be joined to", which is a wider set (`joinableSubjects` admits
  // every content/**.md).
  assert.ok(corpus.tool.length > 0, 'there are tool listings to exclude');
  for (const d of pieces) {
    assert.ok(!d.file.includes('content/directory/tools/'), `${d.file} is a data row, not an entry`);
  }
});
