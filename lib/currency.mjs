...
export function maskNonProse(text) {
  // EVERY mask is matched against the ORIGINAL text, then applied to a buffer.
  //
  // This used to cascade — `out = out.replace(...)` per pattern, so each mask
  // ran against the previous one's output — and that silently swallowed whole
  // lines (beads addictedtoai-2yb, found by the price-attribution check built
  // from a copy of this list). The inline-code mask runs before the
  // indented-code mask, so once a leading span like `openai/gpt-5` was blanked
  // the line BEGAN with twelve spaces, and /^ {4,}\S.*$/gm then read the rest
  // of it as an indented code block and blanked that too:
  //
  //   maskNonProse('`openai/gpt-5`, listed in March, costs $3.00 per million
  //   tokens.')  ->  64 spaces, and findCurrencyLiterals() returned [].
  //
  // A sentence opening with a row id in backticks is the single most common
  // sentence shape in this corpus. Measured impact on the corpus the day it was
  // found was ZERO — latent rather than active — which is exactly why it was
  // worth fixing before it stopped being latent.
  //
  // `split('')` and not `[...text]`: a spread iterates CODE POINTS while
  // `m.index` counts UTF-16 CODE UNITS, so one astral character anywhere in the
  // body would shift every mask after it by one.
