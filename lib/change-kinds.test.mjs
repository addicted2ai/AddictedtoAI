...
test('the closed list is exactly the seven kinds, lead-change and substitution among them', () => {
  assert.deepEqual(
    [...CHANGE_KINDS].sort(),
    ['annotation', 'arrival', 'field_change', 'lead-change', 'release', 'retirement', 'substitution'],
  ...
  assert.ok(
    CHANGE_KINDS.includes('substitution'),
    'a departure the publisher replaced has its own word, so it never renders as "retired"',
  );
