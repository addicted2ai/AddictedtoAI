...
import { later, newest, buildChangedOnMap, reviewedOn, factsMovedOn, contentChangedOn, postChangedOn, indexRouteDates } from './sitemap-dates.mjs';
...
test('indexRouteDates returns each index date from its exact member set', () => {
  const dates = {
    wiki: '2026-01-01',
    learn: '2026-01-02',
    tools: '2026-01-03',
    tutorials: '2026-01-04',
    blog: '2026-01-05',
    deltas: '2026-01-06',
    catalog: '2026-01-07',
    deprecations: '2026-01-08',
    feed: '2026-01-09',
  };
  const content = (doc) => doc.date;
  const post = (doc) => doc.date;
  const changedOn = new Map([
    ['catalog/old', dates.catalog],
    ['catalog/new', '2026-01-10'],
    ['deprecated/old', dates.deprecations],
  ]);
  const site = {
    browsable: [{ date: dates.wiki }, { date: '2025-12-31' }],
    corpus: { learn: [{ date: dates.learn }] },
    tools: [{ doc: { data: { last_verified: dates.tools } } }],
    tutorials: [
      { state: { indexed: false }, doc: { data: { verified_on: '2099-12-31' } } },
      { state: { indexed: true }, doc: { data: { verified_on: dates.tutorials } } },
    ],
    posts: [{ date: dates.blog }],
    deltas: [{ doc: { date: dates.deltas } }],
    catalog: [
      { entry: { id: 'catalog/old' } },
      { entry: { id: 'catalog/new' } },
      { entry: undefined },
    ],
    deprecations: [{ entry: { id: 'deprecated/old' } }],
    changes: [{ date: dates.feed }, { date: '2025-01-01' }],
  };

  assert.deepEqual(indexRouteDates(site, content, post, changedOn), {
    wiki: dates.wiki,
    learn: dates.learn,
    tools: dates.tools,
    tutorials: dates.tutorials,
    blog: dates.blog,
    deltas: dates.deltas,
    catalog: '2026-01-10',
    deprecations: dates.deprecations,
    feed: dates.feed,
    data: '2026-01-10',
  });
});

// Each contributor must be able to determine /data on its own. A single
// fixture with one globally newest member can pass if the fold silently loses
// every other member, so these cases make each route date the sole maximum.
const DATA_FOLD_MEMBERS = ['wiki', 'catalog', 'deprecations', 'deltas', 'feed', 'blog', 'tutorials'];

function dataFoldSite(member) {
  const low = '2026-01-01';
  const high = '2026-02-01';
  const dates = Object.fromEntries(DATA_FOLD_MEMBERS.map((name) => [name, name === member ? high : low]));
  return {
    site: {
      browsable: [{ date: dates.wiki }],
      corpus: { learn: [] },
      tools: [],
      tutorials: [
        { state: { indexed: false }, doc: { data: { verified_on: '2099-12-31' } } },
      ...
      posts: [{ date: dates.blog }],
      deltas: [{ doc: { date: dates.deltas } }],
      catalog: [{ entry: { id: 'catalog/data-fold' } }],
      deprecations: [{ entry: { id: 'deprecated/data-fold' } }],
      changes: [{ date: dates.feed }],
    },
    changedOn: new Map([
      ['catalog/data-fold', dates.catalog],
      ['deprecated/data-fold', dates.deprecations],
    ]),
  };
}

for (const member of DATA_FOLD_MEMBERS) {
  test(`/data date includes the ${member} contributor independently`, () => {
    const { site, changedOn } = dataFoldSite(member);
    const dates = indexRouteDates(site, (doc) => doc.date, (doc) => doc.date, changedOn);
    assert.equal(dates.data, '2026-02-01', `${member} must be observable in the /data fold`);
  });
}

// ---------------------------------------------------------------------------
// EACH ROUTE'S OWN FOLD, member by member (addictedtoai-84s8, HOLE 1).
//
// The /data cases above pin the OUTER fold — that /data sees all seven route
// dates. They say nothing about whether each ROUTE date sees all of ITS OWN
// members, because every route in those fixtures contributes a single member.
// Measured 2026-09-07: mutating the wiki fold to `site.browsable.slice(0, 1)`
// left the sitemap, JSON-LD and date suites at 53 pass / 0 fail. The suite was
// blind to a route silently dropping members, which is the defect 84s8 exists
// for, and it is the same shape as a-dominating-fixture-member-cannot-pin-a-fold
// one level down.
//
// So each route gets a fixture with THREE members where exactly one carries the
// maximum, and the maximum is moved through every position. A fold that reads
// only the first member, only the last, or any proper subset fails on at least
// one position. `newest` ignores order, so no case here depends on ordering.
// ---------------------------------------------------------------------------
const LOW = '2026-01-01';
const HIGH = '2026-02-01';
const POSITIONS = [0, 1, 2];

/** Three dates with the maximum at `winner` and the rest low. */
const trio = (winner) => POSITIONS.map((i) => (i === winner ? HIGH : LOW));

/**
 * One entry per route: how to place three members into a site so the member at
 * `winner` is the sole maximum. `_changedOn` carries the map rows for the two
 * routes that resolve their dates through entry ids rather than through the doc.
 */
const ROUTE_FOLDS = {
  wiki: (d) => ({ browsable: d.map((date) => ({ date })) }),
  learn: (d) => ({ corpus: { learn: d.map((date) => ({ date })) } }),
  tools: (d) => ({ tools: d.map((last_verified) => ({ doc: { data: { last_verified } } })) }),
  tutorials: (d) => ({
    tutorials: d.map((verified_on) => ({ state: { indexed: true }, doc: { data: { verified_on } } })),
  }),
  blog: (d) => ({ posts: d.map((date) => ({ date })) }),
  deltas: (d) => ({ deltas: d.map((date) => ({ doc: { date } })) }),
  feed: (d) => ({ changes: d.map((date) => ({ date })) }),
  catalog: (d) => ({
    catalog: d.map((_, i) => ({ entry: { id: `catalog/m${i}` } })),
    _changedOn: d.map((date, i) => [`catalog/m${i}`, date]),
  }),
  deprecations: (d) => ({
    deprecations: d.map((_, i) => ({ entry: { id: `deprecated/m${i}` } })),
    _changedOn: d.map((date, i) => [`deprecated/m${i}`, date]),
  }),
};

/** An otherwise empty site, so only the route under test can supply a date. */
function emptySite() {
  return {
    browsable: [], corpus: { learn: [] }, tools: [], tutorials: [],
    posts: [], deltas: [], catalog: [], deprecations: [], changes: [],
  };
}

for (const [route, build] of Object.entries(ROUTE_FOLDS)) {
  for (const winner of POSITIONS) {
    test(`${route} fold sees member ${winner} of 3 — a dropped member changes the answer`, () => {
      const { _changedOn, ...overlay } = build(trio(winner));
      const site = { ...emptySite(), ...overlay };
      const changedOn = new Map(_changedOn ?? []);
      const dates = indexRouteDates(site, (doc) => doc.date, (doc) => doc.date, changedOn);
      assert.equal(
        dates[route], HIGH,
        `${route} must fold over every member: the maximum sat at index ${winner}`,
      );
    });
  }
}
