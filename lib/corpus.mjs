...
/**
 * A CLAIM'S SLUG MUST BE UNIQUE ACROSS THE WHOLE CLAIM SET, and this is the
 * check that makes it so.
 *
 * For every other content type the slug is disambiguated by the URL — two files
 * named `speed.md` under different directories become two different pages, and
 * `reviewCandidates` derives its canonical record name from that URL, "the one
 * identifier that is unique by construction" (`lib/reviews.mjs:112-120`). A
 * claim has no URL of its own. Its slug is its file basename, and it is spent
 * twice over:
 *
 *   - as the rendered fragment `#claim-<slug>`, which specs/wiki requires to be
 *     "a stable fragment" — two records with one basename put two elements with
 *     one DOM id on one subject's page, and give both the same `doc.url`;
 *   - as the canonical review-record name `seed-claim-<slug>.md`, so two records
 *     contend for one verdict and whichever the loader reaches last wins.
 *
 * The four-part duplicate check below cannot see either: it keys on subject,
 * field, source_url and accessed, and two records colliding on a basename may
 * legitimately differ in all four. The glob is `claims/**\/*.md`, so nesting is
 * allowed and this is the cost of allowing it — refused here rather than by
 * banning subdirectories, because the collision is real across subjects too
 * (the review name does not mention the subject) and a flatness rule would not
 * catch that.
 */
function checkDuplicateClaimSlugs(claims, diags) {
  const seen = new Map();
  for (const doc of claims) {
    ...
    if (first) {
      diags.error({
        ...
        field: '<filename>',
        message:
          `duplicate claim slug "${doc.slug}" — ${first.file} already uses it. A claim mints no ` +
          'URL of its own, so its basename is what names both its rendered fragment ' +
          `("#claim-${doc.slug}", which must be stable and unique on its subject's page) and its ` +
          `canonical review record ("seed-claim-${doc.slug}.md", which two records would then ` +
          'contend for). Rename one file',
        rule: 'duplicate-claim-slug',
      });
    } else {
      seen.set(doc.slug, doc);
    }
  }
}
...
export async function loadCorpus(opts = {}) {
  ...
  checkDuplicateClaimSlugs(byType.claim, diags);
