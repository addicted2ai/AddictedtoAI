...
export function reviewCandidates(doc) {
  ...
  // A claim mints no route, so its "URL" is a fragment on someone ELSE's page —
  // a poor filename, and absent entirely when its subject did not resolve. Its
  // canonical record name is its own type and slug.
  if (doc.type === 'claim') push(`claim-${doc.slug}`);
  else push(doc.url.replace(/^\//, '').replace(/\//g, '-'));
...
/**
 * The filename a record NAME refers to, normalised.
 *
 * A `reads-human-from` entry names the record its voice verdict stands on, and
 * a reviewer reading `data/reviews/` writes that name the way it sees it: with
 * or without the `.md`, occasionally with the directory in front. All three
 * mean one file. Normalised HERE, in the module that owns the record store, so
 * the merge gate (which reads the directory) and the launch check (which reads
 * the loaded map) resolve a name identically — two resolvers that agree today
 * are how the two ends drift apart later.
 */
export function recordFileName(name) {
...
/**
 * Does this record name this content path as something it reviewed?
 *
 * Read through the record's own declarations — the front-matter subject keys
 * (`subjectsOf`) and the `reviewed:` map (`reviewedOf`) — rather than by a
 * string compare on one subject line, so a record that names several pieces, or
 * names them under any of the nine accepted keys, answers correctly.
 *
 * Used by the carry-forward anchor check at both ends (specs/review,
 * addictedtoai-37rb): the record a carry-forward stands on must approve THE
 * POST THAT ENTRY NAMES, not merely exist.
 */
export function recordNamesPath(rec, path) {
  const want = String(path ?? '').replace(/\\/g, '/');
  if (!want) return false;
  if (subjectsOf(rec).includes(want)) return true;
  return Object.prototype.hasOwnProperty.call(reviewedOf(rec), want);
}
 ...
 *
 * CLAIMS ARE APPENDED AT THE END, and the position is the reason they can be
 * added at all: because the order is part of the join, inserting a new type
 * anywhere but the end would move which piece claims an ambiguously-named
 * record, changing verdicts for content nobody touched. Appending cannot.
 *
 * A claim record is reviewable — it has to be, and that is most of why the
 * record sits beside the entry rather than on it (specs/wiki,
 * `separate-a-claim-from-a-fact`). A verification landing next month dirties
 * THAT RECORD'S hash and demands a fresh verdict on it, which is correct
 * because the verification is the judgment; carried in the entry's front
 * matter, the same event would have marked a page of untouched prose
 * mismatched. A claim has no prose body and no indexability, so it appears
 * here only for the hash binding.
 *
 * BODY-LESS ENTRIES ARE APPENDED LAST, and for exactly the reason claims are
 * (specs/review, beads addictedtoai-kpgn). A body-less entry still carries
 * sourced facts, in front matter, which is INSIDE the reviewed surface — so a
 * fact-only edit to one moves that surface exactly as a prose edit moves a
 * page's. Filtered out, that class of edit produced no signal in any of the four
 * states: not missing, not unbound, not mismatched, absent from the count
 * entirely, at any layer. Measured 2026-09-06: 458 of 553 entries have no prose
 * body, including all 38 `tool` entries, and `content/wiki/tool/vllm.md` had a
 * sourced fact's value changed direct to `main` in ba1a577 with no record
 * anywhere.
 *
 * `corpus.entry.filter((d) => d.hasBody)` stays FIRST and unmoved, and these are
 * appended after `corpus.claim`, because the order is part of the join: a record
 * is claimed by the first piece that names it, so inserting these anywhere
 * earlier would move which piece claims an ambiguously-named record for content
 * nobody touched.
 *
 * TOOL LISTINGS STAY OUT, unchanged and deliberately — `content/directory/tools/`
 * rows are data, not entry facts, the set has excluded them since it was
 * written, and the requirement says "every wiki entry" rather than "every content
 * file a record can be joined to". Those two sets are not the same set:
 * `joinableSubjects` in the merge admits every `content/**.md`.
 *
 * Extending the BINDING is not extending the obligation to HAVE a record.
 * `verify-launch` keeps its own prose bar for the pieces it refuses to launch
 * without one, and a body-less piece reporting `missing` fails nothing — on the
 * same terms and for the same reason `unbound` fails nothing.
 */
 ...
 * the reader to ignore the report. An UNORDERABLE tie is different — it keeps
 ...
 *
 * SUPERSESSION IS PER PIECE. The map's value is the SET of files a newer record
 * beat this one to, not a single file, because a record that reviewed several
 * pieces can be superseded on some and still be the only review the others have
 * ever had. Keyed by record name alone, one re-review disowned every other page
 * the same record had approved — see the note at `addDeclared`.
 *
 * @returns {{byFile: Map<string, object>, claimed: Map<string, string>,
 *            superseded: Map<string, Set<string>>, orphans: string[],
...
export function resolveReviews(pieces, records, { hashOf } = {}) {
  ...
  /**
   * record name -> ONE file, the LAST piece this record bound.
   *
   * Documented rather than widened (addictedtoai-15ar): since `addDeclared` a
   * record naming several pieces in `subject:` binds all of them, so this map
   * loses every piece but the last. Its only consumers ask boolean questions —
   * `taken()` ("is this record used at all") and the contention message — so
   * nothing today reads the lost files. **Do not read `claimed.get(name)` as
   * "the piece this record reviewed".** For that question read `byFile`, whose
   * `declaredBy` carries every record that named a piece.
   */
  const claimed = new Map();
  /**
   * record name -> the set of FILES a newer record beat it to.
   *
   * A Set per record, not one file per record: a record naming several pieces
   * can legitimately be superseded on some of them and still be the only
   * review the rest have ever had. `taken()` and the returned map still read
   * membership by record name, which is the question those ask.
   */
  const superseded = new Map();
  const supersedeFor = (name, file) => {
    if (!superseded.has(name)) superseded.set(name, new Set());
    superseded.get(name).add(file);
  };
  const supersededFor = (name, file) => Boolean(superseded.get(name)?.has(file));
  /**
   * record name -> the set of FILES on which this record sat in an unorderable
   * tie: bound to nothing there, already reported once.
   *
   * PER PIECE, exactly as `superseded` is, and for the same reason
   * (addictedtoai-15ar, the third site of the shape addictedtoai-sge and
   * addictedtoai-9ls8 each cost a `verify-launch` failure). A Set of names
   * disowned a record on EVERY piece it declared the moment it tied on ONE —
   * including pieces where it is the only review that ever existed, which then
   * read `missing`. `taken()` and orphan detection still ask by name, because
   * "is this record used anywhere" is the question those ask.
   */
  const unorderable = new Map();
  const unorderableFor = (name, file) => Boolean(unorderable.get(name)?.has(file));
  const markUnorderable = (name, file) => {
    if (!unorderable.has(name)) unorderable.set(name, new Set());
    unorderable.get(name).add(file);
  };
  const contended = [];
  ...
  /**
   * "Is this record used ANYWHERE?" — by name, deliberately, and its only
   * consumer is orphan detection below, where that is exactly the question. A
   * record that tied on one piece is not an orphan even though it bound none.
   *
   * The per-piece questions do NOT go through here (addictedtoai-15ar): both
   * `add` and `addDeclared` read `unorderable` per (record, piece) instead,
   * because a tie on one piece is not a verdict about the others.
   */
  ...
  for (const doc of pieces) {
    ...
    const add = (rec, matchedBy) => {
      // The `unorderable` half is read PER PIECE; `claimed` and `superseded`
      // stay by name, which is what those two questions mean here.
      //
      // This is `taken()` spelled out rather than called, and the difference is
      // the second half of addictedtoai-15ar. `taken()` answers "is this record
      // used ANYWHERE" — the right question for orphan detection below, and the
      // wrong one here. Reading `unorderable` by name refused a record on the
      // piece whose CANONICAL FILENAME it is the moment it tied on some OTHER
      // piece it had declared in `subject:`; that piece then bound nothing and
      // read `missing`, though this record was the only review it ever had.
      //
      // MEASURED on `lib/fixtures/review-states` before the fix: a record named
      // `seed-wiki-concept-unbound.md`, carrying `unbound`'s current hash and
      // also declaring `mismatched` in `subject:`, tied with a rival on
      // `mismatched` — and `unbound` reported `missing` with `byFile` binding
      // nothing. The `addDeclared` twin of this line was converted first; the
      // filename path reached the same Map through `taken()` and was not.
      if (claimed.has(rec.name) || superseded.has(rec.name)) return;
      if (unorderableFor(rec.name, doc.file)) return;
    ...
    const addDeclared = (rec, matchedBy) => {
      // PER PIECE, not per record. `superseded` records that a NEWER review
      // won a particular file; it says nothing about the other files the same
      // record reviewed. Keying this check on the record name alone made one
      // job's record lose EVERY page it reviewed the moment any single one of
      // them was re-reviewed. `unorderable` is read per piece for the identical
      // reason (addictedtoai-15ar): a tie on ONE piece is not a verdict about
      // the others, and keyed by name it disowned pieces whose only review this
      // record was.
      //
      // MEASURED 2026-09-04. `j-20260903-10.md` reviewed and approved two
      // pages in one job. A repair on 2026-09-04 re-reviewed only the model
      // page, superseding that record there — and `content/wiki/org/inception-labs.md`
      // then reported `missing`, i.e. never reviewed, though this record named
      // it in `subject:`, carried its hash in `reviewed:`, and that hash still
      // matched the file byte for byte. `verify-launch` failed on 163/164.
      //
      // It was also order-dependent, which is why it had not fired before: had
      // the org page been visited first it would have bound the record and the
      // model page would have taken the newer one, and nothing would have been
      // wrong. The same defect that produced `addDeclared` (addictedtoai-sge)
      // in the first place, one layer along: the more pieces one record covers,
      // the more of them the join can disown.
      if (supersededFor(rec.name, doc.file) || unorderableFor(rec.name, doc.file)) return;
      if (found.some((f) => f.rec.name === rec.name)) return;
      found.push({ rec, matchedBy });
    };
    ...
    if (found.length === 1) {
      ...
      if (ordered) {
        chosen = ordered[0];
        for (const f of ordered.slice(1)) supersedeFor(f.rec.name, doc.file);
      } else {
        const names = found.map((f) => f.rec.name).sort();
        for (const n of names) markUnorderable(n, doc.file);
        contended.push(
