 ...
 * ratchet pass their own list. `censusDebt` is empty for the identical
 * reason, and `censusSnapshotDate` defaults to `null` (unverifiable — the
 * default `demoDataLayer()` has no `openrouter-models` source at all) rather
 * than to some fixed date, so a fixture's dated-mismatch tests have to pass
 * the date explicitly and mean what they say.
 */
export function buildFixture(name, opts = {}) {
  return buildSite({
    ...
    censusDebt: opts.censusDebt ?? { known: [] },
    censusSnapshotDate: opts.censusSnapshotDate ?? null,
    ...opts,
  });
}
