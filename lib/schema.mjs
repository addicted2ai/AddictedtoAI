...
export const postSchema = z
  ...
  .strict()
  .superRefine((v, ctx) => {
    // The build gate. It runs inside the `content` prebuild step, which already
    // reports the file and the field, so the gate needs no new step and no new
    // entry point — the `path` on each issue is what names the offending field.
    if (v.frontier === true && (v.frontier_reason ?? '') === '') {
      ctx.addIssue({
        code: 'custom',
        path: ['frontier_reason'],
        message:
          'a post declaring `frontier: true` must cite exactly one criterion in '
    ...
    // A criterion is checked whenever one is DECLARED, flagged or not: `F6` is
    // not a criterion whatever the flag says, and a post carrying a criterion
    // it never claimed is a mistake worth stopping on rather than ignoring.
    if (v.frontier_reason !== undefined && !FRONTIER_REASONS.includes(v.frontier_reason)) {
      ctx.addIssue({
        code: 'custom',
        path: ['frontier_reason'],
        message:
          `invalid frontier_reason ${JSON.stringify(v.frontier_reason)} — the criterion `
          + `must come from the closed list: ${FRONTIER_REASONS.join(', ')}`,
      });
    }
    for (const [i, d] of (v.domains ?? []).entries()) {
