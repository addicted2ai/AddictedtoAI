...
/**
 * The closed category list for directory tool listings (specs/directory,
 * beads addictedtoai-0eg).
 *
 * Closed for the same reason `KINDS` is closed: an open field drifts into
 * `coding` / `code` / `Coding` and the grouping stops being a partition. A
 * value outside this list fails the build naming the file and the value.
 *
 ...
 * reason `lib/units.mjs` states about units and `aliases[].class` states about
 * alias classes: a heuristic that is right 90% of the time is a heuristic that
 * is silently wrong 10% of the time, and nothing downstream can tell which.
 *
 ...
 * this array cannot move a category up the page. That matters because
 * specs/directory promises placement is never sold: a hand-ordered array would
 * be exactly the kind of ordering the promise forbids, and the guarantee has to
 * be a mechanism rather than a convention about how this list is edited.
 *
 * `video` is deliberately absent. No listing in the corpus has video as its
 * primary job — ComfyUI does image and video and is categorised `image` — and a
 * category with no members is a promise the directory does not keep. Adding it
 * is one line here plus the gloss in `lib/listings.mjs`, and the mechanism that
 * pairs those two is checked (`categoryProblems`).
 */
