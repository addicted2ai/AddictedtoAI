...
import { DOMAINS, frontierFlagProblems } from './domains.mjs';

export { TOOL_CATEGORIES, DOMAINS };
...
/**
 * The four refusals `checkPublishesFromValue` can return, each said in the
 * terms the editor needs to act on it.
 *
 * Two of them have to say something other than "declare the reduction", and for
 * the same reason: a reduction is a remediation, and a remediation that names a
 * string nothing can ever match is the build teaching the defect.
 *
 * - `suffixShaped` — the reduction IS the public suffix, so `vendor.co.il` was
 *   refused with *declare `co.il` instead, which covers every host under it*
 *   before that branch existed.
 * - `notAHost` — a URL, a path, a port or a space has no reduction to offer.
 *   `platform.kimi.ai/blog` reduced to `kimi.ai/blog`, and declaring THAT
 *   matches no host any claim will ever carry, so the field would read as
 *   declared and do nothing (red-team FM-N6, measured on this tree).
 */
function publishesFromMessage(id, value, check) {
  const head = `entry ${JSON.stringify(id)}: publishes_from value ${JSON.stringify(value)}`;
  if (check.notAHost !== undefined) {
    return (
      `${head} is not a hostname — a scheme, a path, a port, a space or a trailing dot is not a ` +
      'domain. Declare the bare registrable domain: "kimi.ai", never "https://kimi.ai/blog". ' +
      'The vendor test compares a claim\'s source_host against this value as a registrable ' +
      'domain, so a value that is not one matches nothing and the declaration is inert'
    );
  }
  if (check.suffixShaped) {
    return (
      `${head} reduces to ${JSON.stringify(check.suffixShaped)}, which has the shape of a PUBLIC ` +
      'SUFFIX that lib/vendor-domain.mjs\'s MULTI_LABEL_SUFFIXES does not list. Declaring a ' +
      'public suffix would attribute every registrant under it to this entry. Add the suffix to ' +
      'MULTI_LABEL_SUFFIXES, then declare the registrable domain beneath it'
    );
  }
  if (check.reduction) {
    return (
      `${head} is a host, not a registrable domain — declare ` +
      `${JSON.stringify(check.reduction)} instead, which covers every host under it`
    );
  }
  return `${head} has no registrable domain — it is a bare public suffix or not a host at all`;
}
...
export const entrySchema = z
  .object({
    ...
    // ---- the domain facet (change `tag-the-corpus-by-domain`) --------------
    //
    // What a thing is FOR, beside the `kind` that says what it IS. Set-valued,
    // optional, empty legal and common: "general" is the UNMARKED default (K38),
    // so there is no `general` value to declare and no `text` value either.
    // The vocabulary is `lib/domains.mjs` and is read, never restated — two
    // closed lists of the same eight strings drift, and the moment they do the
    // post gate and the entry gate are two different checks wearing one name.
    //
    // THREE KEYS, ONE REGIME EACH, and the split is mechanical rather than
    // tidy. `MECHANICAL_FRONT_MATTER_KEYS` is matched by key NAME across every
    // content kind (`lib/review-hash.mjs:99-102`), so one `domains` field
    // carrying both a machine's re-seeds and an editor's judgment has two
    // possible fates and both are defects: on the mechanical list a judgment
    // publishes unreviewed (and a POST's editorial `domains` is silently
    // exempted with it); off it, every re-seed marks 544 entries mismatched on
    // prose nobody touched.
    //
    //   `domains_seeded`   machine-written by the Pulse from named feed fields,
    //                      APPEND-ONLY (K47), on MECHANICAL_FRONT_MATTER_KEYS
    //   `domains`          editorial additions, inside the reviewed surface
    //   `domains_excluded` editorial removals, inside the reviewed surface
    //
    // The effective set is `(domains_seeded ∪ domains) − domains_excluded`.
    // `.optional()` and not `.default([])`: an absent key must stay absent in
    // the file's own front matter, because a zod default would put an empty
    // `domains_seeded` on every entry the moment anything serialised `data`.
    domains: z.array(closedList('domain', DOMAINS)).optional(),
    domains_excluded: z.array(closedList('domain', DOMAINS)).optional(),
    domains_seeded: z.array(closedList('domain', DOMAINS)).optional(),
  ...
  .superRefine((v, ctx) => {
    ...
    (v.publishes_from ?? []).forEach((value, i) => {
      ...
      ctx.addIssue({
        ...
        message: publishesFromMessage(v.id, value, check),
      });
    });
    ...
    // ---- the two domain gates ---------------------------------------------
    //
    // BOTH READ ONLY THE FRONT MATTER OF THE FILE BEING VALIDATED, never the
    // current snapshot. That is what keeps an editorial key uncoupled from the
    // feed, and it holds only because seeding is APPEND-ONLY: `domains_seeded`
    // is an accumulated record of every signal ever observed, not a view of
    // today's rows, so a publisher dropping a signal removes nothing and cannot
    // turn a green build red on an entry nobody touched. Measured 2026-09-05:
    // one publisher's index rebase dropped `agentic_index` presence 166 -> 99
    // between two committed snapshots.
    const asDomains = (x) => (Array.isArray(x) ? x.filter((d) => typeof d === 'string') : []);
    const asserted = asDomains(v.domains);
    const seeded = asDomains(v.domains_seeded);
    const suppressible = new Set([...seeded, ...asserted]);
    const assertedSet = new Set(asserted);
    asDomains(v.domains_excluded).forEach((value, i) => {
      const where = { code: 'custom', path: ['domains_excluded', i] };
      if (assertedSet.has(value)) {
        // A contradiction, not a precedence question: resolving it silently in
        // either direction would hide an editing mistake.
        ctx.addIssue({
          ...where,
          message:
            `entry ${JSON.stringify(v.id)}: domain ${JSON.stringify(value)} is declared in BOTH ` +
            '`domains` and `domains_excluded` — asserting and excluding one value is a ' +
            'contradiction, not a precedence question; remove it from one of the two',
        });
        return;
      }
      if (!suppressible.has(value)) {
        // An exclusion that removes nothing is a STALE EDIT: it reads as a
        // decision and enacts none, which is the shape this repository keeps
        // catching. The ordering it imposes is stated rather than discovered —
        // an exclusion follows the value it suppresses and never precedes it.
        ctx.addIssue({
          ...where,
          message:
            `entry ${JSON.stringify(v.id)}: domains_excluded lists ${JSON.stringify(value)}, which ` +
            'appears in neither `domains_seeded` nor `domains` — an exclusion that removes nothing ' +
            'is a stale edit. Remove it, or write it after the seeding run that adds the value. ' +
            `Suppressible on this entry: ${[...suppressible].sort().map((d) => JSON.stringify(d)).join(', ') || '(none)'}`,
        });
      }
    });
  });
...
export const postSchema = z
  .object({
    ...
    corrections: z
      ...
      .default([]),
    // ---------------------------------------------------------------------
    // THE FRONTIER FLAG (flag-what-moved-the-frontier, specs/blog "A frontier
    // flag is earned, declared, and gated at the build").
    //
    // Three keys, and the shapes here are deliberately loose because the RULE
    // that binds them is conditional and lives in the `superRefine` below: a
    // field-level `closedList` on `frontier_reason` could not express "required
    // when, and only when, `frontier` is true", and a required field would
    // reject every unflagged post in the corpus.
    //
    // These three keys are EDITORIAL. They are deliberately NOT added to
    // `MECHANICAL_FRONT_MATTER_KEYS` (`lib/review-hash.mjs`): tagging a
    // published post changes its reviewed surface, its record reports
    // `mismatched`, and it is corrected by review rather than by exemption.
    // What a story is, and where it lands, is exactly the judgment this site
    // does not let publish unreviewed. `lib/review-hash.test.mjs` asserts the
    // absence, because the §3 wiki facet adding a machine-seeded key literally
    // named `domains` to that list would silently exempt a POST's editorial
    // `domains` from review — the list is matched by key NAME across every
    // content kind, with no per-kind scoping.
    frontier: z.boolean().default(false),
    frontier_reason: z.string().optional(),
    // Optional, flagged or not, and an ABSENT or EMPTY value is not a defect:
    // absence IS the vocabulary's unmarked "general" (K38, and keeper ruling
    // K46 on `loops/ui-loop/graph/artifacts/BLIND-002.md`, which withdrew an
    // earlier "at least one domain" bar). A court filing, a regulator's
    // action, a licence revenue gate and a system card are F4/F5-shaped events
    // that map to no value in the vocabulary; a gate failing them would make
    // the criteria unreachable on the one surface the flag exists to populate.
    // `.default([])` is what makes `domains: []` and an absent key the same
    // parsed value, which is what "an empty list means what an absent key
    // means" asks for.
    domains: z.array(z.string()).default([]),
  })
  .strict()
  .superRefine((v, ctx) => {
    // THE BUILD GATE, and it is one call rather than a copy of the rule. The
    // same three keys are judged at the scout's merge over a candidate that is
    // not a post yet (`loop/lib/proposals.mjs`), and two copies of one closed
    // rule drift silently — after which the filing gate and the build gate are
    // two different checks wearing one name.
    //
    // This runs inside the `content` prebuild step, which already reports the
    // file and the field, so the gate needs no new step and no new entry point:
    // the `path` on each issue is what names the offending field.
    for (const p of frontierFlagProblems(v)) {
...
export const toolSchema = z
  .object({
    ...
    // The fields of AI the tool serves — a second axis beside `category`, which
    // is the JOB it is for. Different questions, and neither answers the other:
    // measured 2026-09-05, 23 of the 35 curated listings sit in categories
    // (`local`, `inference`, `training`, `observability`, `data`, `frameworks`,
    // `evaluation`) that are performed across every domain and belong to none.
    // So it is optional and the empty set is the common case, not an omission to
    // be chased.
    //
    // `domains` ONLY — deliberately not `domains_seeded` and not
    // `domains_excluded`. No feed seeds a tool listing, so there is nothing to
    // exclude and no machine value to exempt from review; a key that can never
    // do anything is a key that will be misread. Every value here is a judgment
    // and publishes through review, which is what keeps it off
    // `MECHANICAL_FRONT_MATTER_KEYS`.
    //
    // Editorial and DECLARED: never derived from `category` by a lookup table.
    // A category-to-domain mapping is a heuristic wearing a configuration
    // file's clothes — `coding` the category and `coding` the domain agree by
    // coincidence of naming, and `retrieval` -> `research` is a judgment
    // somebody has to make and be accountable for.
    domains: z.array(closedList('domain', DOMAINS)).optional(),
    mentions: z.array(entryId).default([]),
...
export const NON_PROSE_FIELDS = Object.freeze({
  entry: Object.freeze({
    ...
    'themes[]': 'a theme name',
    'domains[]': 'a closed-list value from lib/domains.mjs — an editorial facet id, not a sentence',
    'domains_excluded[]': 'a closed-list value — an editorial suppression of one facet id',
    'domains_seeded[]':
      'MECHANICALLY MAINTAINED — written by the Pulse from named feed fields under the review ' +
      'exemption (it sits beside `timeline` in MECHANICAL_FRONT_MATTER_KEYS), so it is not ' +
      'author-written at all',
    'publishes_from[]':
  ...
  post: Object.freeze({
    ...
    frontier_reason:
      'a closed-list value — one of the five frontier criteria ids (lib/domains.mjs), not a '
      + 'sentence saying why; the reason a criterion states is normative text in specs/blog and '
      + 'the post cites it by id',
    'domains[]':
      'a closed-list value from the domain vocabulary (lib/domains.mjs), and a facet is a tag '
      + 'rather than an author-written clause',
  }),
  tool: Object.freeze({
    title: 'a name',
    url: 'a URL',
