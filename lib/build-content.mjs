...
export async function buildSite(opts = {}) {
  ...
  for (const doc of corpus.all) {
    ...
    if (sc.scanned > 0) {
      ...
      snapshotCensus.hedged += sc.hedged ?? 0;
...
export async function contentBuildStep() {
  ...
  // `hedged` is printed for the same reason (beads addictedtoai-jqif): a claim
  // that stops being an error must not stop being visible, or the hedge branch
  // could swallow the corpus and this line would read like a clean one.
  ...
  process.stdout.write(
    `prebuild: snapshot census — bound to ${site.censusSnapshotDate ?? '(no snapshot yet)'}; ` +
      ...
      `error), ${sc.hedged} hedged as dated observations, ${sc.known} recorded as debt (warning; ` +
