...
test('2.9 a redirect entry appears in the generated vercel.json', () => {
  ...
  // `headers` passed explicitly so this stays a test about REDIRECTS. The
  // generator's default is the CORS block (addictedtoai-k1j), which
  // lib/redirects.test.mjs owns.
  const vercel = mergeVercelConfig({ cleanUrls: true }, rules, []);
  assert.deepEqual(vercel, { cleanUrls: true, headers: [], redirects: rules });
});
...
test('2.9 an existing vercel.json keeps its other top-level keys', () => {
  // `headers` used to be the example of an unowned key here. It is a GENERATED
  // key now (the CORS block, addictedtoai-k1j) and is regenerated rather than
  // preserved, deliberately — a `headers` array that survived because it
  // happened to be in the file could not be corrected by a build. So the
  // preservation claim is made with a key the generator does not own.
  const merged = mergeVercelConfig({ cleanUrls: true, trailingSlash: false, redirects: [] }, [
    { source: '/a', destination: '/b', permanent: true },
  ]);
  assert.equal(merged.cleanUrls, true);
  assert.equal(merged.trailingSlash, false);
