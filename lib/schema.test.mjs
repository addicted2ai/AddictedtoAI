...
test('F1-F5 the criterion is refused for a NEAR miss too, through the build gate itself', () => {
  // `F6` above is a far miss: it shares no prefix, suffix, substring or case
  // with any real id, so a membership test loosened to `startsWith`,
  // `endsWith`, `includes`, a trim or a case fold would keep refusing it and
  // this block would stay green. `lib/domains.test.mjs` carries the full
  // battery against the shared rule; these three go through
  // `validateFrontMatter` because the build gate is where a post's bytes are
  // actually judged, and a loosening measured only on the helper leaves the
  // caller unproven.
  //
  // `F2 — a rescoring that moved a leader` is the one an author actually
  // types, and it is exactly the prose form `NON_PROSE_FIELDS.post
  // .frontier_reason` forbids: the post cites the criterion by id, and the
  // sentence saying why belongs in the body.
  for (const miss of ['F1x', 'f2', 'F2 — a rescoring that moved a leader']) {
    const res = validateFrontMatter('post', post({ frontier: true, frontier_reason: miss }));
    assert.equal(res.ok, false, `${JSON.stringify(miss)} is not a criterion — membership must be exact`);
    assert.deepEqual(fieldsOf(res), ['frontier_reason'], JSON.stringify(miss));
  }
  // The control on the same assertion: every real id still validates, so the
  // refusals above are exactness and not a gate that refuses everything.
  for (const id of FRONTIER_REASONS) {
    const ok = validateFrontMatter('post', post({ frontier: true, frontier_reason: id }));
    assert.equal(ok.ok, true, `${id} must still validate: ${JSON.stringify(ok.issues)}`);
  }
});
