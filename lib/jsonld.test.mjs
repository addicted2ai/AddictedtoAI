...
import { indexRouteDates } from './sitemap-dates.mjs';
...
test('index graphs carry the dateModified supplied by the shared sitemap decision', () => {
  const site = {
    browsable: [{ date: '2026-08-31' }],
    corpus: { learn: [] },
    tools: [],
    tutorials: [],
    posts: [],
  ...
  const resolver = (doc) => doc.date;
  const dates = indexRouteDates(site, resolver, resolver, new Map());
  assert.equal(
    definedTermSetGraph([entryDoc()], { dateModified: dates.wiki }).dateModified,
    dates.wiki,
  );
  assert.equal(datasetGraph({ dateModified: dates.data }).dateModified, dates.data);
});

test('index graphs omit dateModified when the caller supplies none — never a build clock', () => {
