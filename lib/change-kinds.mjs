 ...
 * this change.
 *
 * ## `substitution`, the seventh (2026-09-07)
 *
 * A departed feed row was one thing to this list and one word to a reader:
 * `retirement`, rendered "retired". On 2026-09-05 that word was false about one
 * of the two departures — Alibaba moved the undated `qwen3.8-max` name onto a
 * newer snapshot, so the row left `openrouter-models` while the model line
 * continued and the old slug became a 307 to `qwen/qwen3.8-max-0902`. The
 * changed feed printed "Qwen: Qwen3.8 Max (0902) arrival" and "Qwen: Qwen3.8
 * Max retired" adjacent, and only the first was true.
 *
 * A departure and a substitution are different events and the history needs
 * different words for them. `pulse/lib/diff.mjs` decides which one a departure
 * is, from the two snapshots alone, under the registry's `substitution_rule`;
 * a source with no such rule emits `retirement` exactly as before.
 *
 ...
 *
 * It is the order `specs/pulse` names them, followed by the kinds added since,
 * newest last: `lead-change`, then `substitution`. Nothing sorts or ranks by it
 * — it is a reading order, and `unrecognisedKinds` below is the only consumer
 * that iterates the array at all.
 *
