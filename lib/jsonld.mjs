 ...
 *
 * **The two index-level graphs carry the shared `dateModified`.** Callers pass
 * the member-max values returned by `indexRouteDates()` in
 * `lib/sitemap-dates.mjs`, which is the same computation the sitemap uses.
 * This file consumes that decision but never reimplements it, so `/wiki` and
 * `/data` cannot disagree with their sitemap `<lastmod>` values.
 *
...
/** The `/wiki` index as the set those terms belong to. */
export function definedTermSetGraph(terms, { description, dateModified } = {}) {
  return compact({
    '@context': SCHEMA_CONTEXT,
    '@type': 'DefinedTermSet',
    '@id': DEFINED_TERM_SET_URL,
 ...
 * that stops being written stops being advertised in the same edit. The
 * caller supplies the shared `/data` index date from `indexRouteDates()`.
 */
export function datasetGraph({ dateModified } = {}) {
