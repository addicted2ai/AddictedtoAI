...
export async function buildRouteTable(corpus, opts = {}) {
  ...
  //
  // NO `?? corpus.all` FALLBACK. It was written here as a courtesy to a caller
  // holding an older corpus shape and it silently restored the exact leak this
  // line was changed to close: with `documents` missing, `all` is walked, and
  // `/wiki/org/x#claim-y` enters the table again with nothing failing. The
  // carve-out's whole design is the `routed` flag, so a corpus that predates it
  // is a caller to fix, not a case to serve.
  if (corpus && !corpus.documents) {
    throw new Error(
      'buildRouteTable: this corpus has no `documents` — it predates the `routed` flag in ' +
        'CONTENT_TYPES (lib/paths.mjs). Load it with loadCorpus(); walking `all` would add a ' +
        'route-less type\'s URL to the route table, and a claim\'s URL is a fragment',
    );
  }
  for (const doc of corpus?.documents ?? []) routes.add(normalizeRoute(doc.url));
