...
test('every word of the generic family is excluded, including bare "seed"', () => {
  for (const word of ['ai', 'labs', 'lab', 'cloud', 'inc', 'corp', 'research', 'seed', 'group', 'technologies']) {
...
test('bare seed is not ByteDance Seed ownership, while its declared brand is', () => {
  const org = {
    display_name: 'ByteDance Seed',
    aliases: [{ name: 'ByteDance Seed Team', class: 'exclusive' }],
    publishes_from: ['byteplus.com'],
  };
  assert.equal(isSubjectOwned('seed.example', org), false);
  assert.equal(isSubjectOwned('ai.byteplus.com', org), true);
});
