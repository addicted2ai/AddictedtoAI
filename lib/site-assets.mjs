 ...
 *   /robots.txt                  the crawler stance, as a commented decision
 *   /llms.txt                    where the substance is, for a model
 *                                (both: lib/crawlers.mjs, addictedtoai-k1j)
...
import { renderRobotsTxt, renderLlmsTxt } from './crawlers.mjs';
 ...
 *
 * `schema_version` and `contract` are what turn this from a file that happens
 * to be fetchable into a **published contract** somebody else's project can
 * depend on (beads addictedtoai-k1j). The version is the only thing a consumer
 * can act on programmatically, and it means exactly one thing: a key was
 * renamed or removed. See `lib/asset-routes.mjs` for what is stable, what is
 * deliberately not, and why an ADDED key does not bump it.
 */
...
export async function writeSiteAssets(site, opts = {}) {
  ...
  // ---- what a machine arriving without a browser is told (addictedtoai-k1j)
  // Both are written here, after the dataset, and `llms.txt` takes the
  // dataset's own `counts` rather than recounting anything: the two files
  // state the same numbers because they are the same numbers, and
  // `verify-surfaces` measures that against the exported payload.
  await writeText(ROBOTS_ROUTE, renderRobotsTxt());
  written.push(ROBOTS_ROUTE);
  await writeText(LLMS_ROUTE, renderLlmsTxt(dataset.counts));
  written.push(LLMS_ROUTE);

  // The IndexNow key file: the key, and nothing else — no newline, no BOM, no
  // wrapper. A search engine fetches this URL and compares its whole body to
  // the key in a submission, so anything else in the file is a rejection.
  // Public by design (lib/asset-routes.mjs) and written on every build, so the
  // host can never be advertising a key it does not serve.
  await writeText(INDEXNOW_KEY_ROUTE, INDEXNOW_KEY);
  written.push(INDEXNOW_KEY_ROUTE);
