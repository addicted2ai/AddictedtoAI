...
export async function writeSiteAssets(site, opts = {}) {
  ...
  const stamp = buildStamp({ dirty: opts.dirty });
 ...
 * the `content` step ahead of it rewrites eleven git-tracked files under
...
export async function siteAssetsStep(opts = {}) {
  ...
  const { stamp, written, dataset } = await writeSiteAssets(site, { dirty: opts.dirty });
