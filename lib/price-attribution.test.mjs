...
// ---------------------------------------------------------------------------
// THE QUOTE-AWARE SENTENCE BOUNDARY (addictedtoai-q3n)
// ---------------------------------------------------------------------------

test('q3n a full stop inside a closing quote ends the sentence', () => {
  // A `[.!?](?=\s|$)` boundary does not split at `...in 2026."` because the
  // next character is the quote mark, so the two sentences below read as one
  // and the SECOND — which merely states a row — inherits the FIRST's
  // attributing verb. Measured in arxiv-pin.mjs, filed here as q3n.
  const s =
    'The analyst wrote that the lab "charges whatever the market will bear for reasoning ' +
    'models in 2026." The input row is {{fact:model/priced-model#price_input}}.';
  assert.deepEqual(
    findPriceAttribution(s, 1),
    [],
    'the row sentence must not inherit the quoted sentence\u2019s verb',
  );
  const second = sentenceAround(s, s.indexOf('{{fact:'));
  assert.doesNotMatch(second.text, /charges/, 'the quoted sentence is a separate sentence');
  // THE OTHER HALF OF THE BOUNDARY, and it needs its own assertion: the class
  // decides WHERE the split lands, the arithmetic `m.index + m[0].length`
  // decides which side the closers fall on. With the pre-fix `m.index + 1` the
  // class still matches, the two sentences still split, and every assertion
  // above still passes — but the closing quote is handed to the NEXT sentence,
  // which is precisely the defect the bead describes. `lib/arxiv-pin.test.mjs`
  // pins the same arithmetic with `/before 2026\."$/`; mirrored here so a later
  // reader who "simplifies" the length back to 1 fails a test rather than
  // shipping a silently mis-split corpus.
  assert.match(
    sentenceAround(s, 0).text,
    /in 2026\."$/,
    'the closing quote belongs to the sentence it closes',
  );
});

test('q3n closing brackets after the stop also end the sentence', () => {
  // The corpus ends price sentences `... (top provider's rate).` and, less
  // often, with the closer AFTER the stop — both must bound.
  const s =
    'The provider layer is where a rate is set (top provider\u2019s rate.) The batch row is ' +
    '{{fact:model/priced-model#price_input}}.';
  const second = sentenceAround(s, s.indexOf('{{fact:'));
  assert.doesNotMatch(second.text, /rate/, 'the parenthesised sentence ended at `.)`');
});

test('q3n an unquoted sentence still bounds exactly as it did', () => {
  // The control: the quote-aware class must not move any boundary that the
  // old one already found.
  const s = 'It charges a lot. The row is {{fact:model/priced-model#price_input}}.';
  assert.deepEqual(findPriceAttribution(s, 1), []);
  assert.equal(sentenceAround(s, 0).text, 'It charges a lot.');
});
