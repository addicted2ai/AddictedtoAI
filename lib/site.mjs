...
import { postsNewestFirst, warnPostCeiling } from './posts.mjs';
...
async function assemble(opts = {}) {
  ...
  // Two gates that need a whole-surface view and therefore could not run in
  // the per-document pass: the learn ladder's prerequisite graph (a cycle
  // makes the generated index arbitrary) and the blog ceiling (a rolling
  // window over every published date).
  checkPrerequisiteCycles(corpus.learn, site.diags);
  warnPostCeiling(corpus.post, site.diags);
