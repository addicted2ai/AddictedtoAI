...
import { buildChangedOnMap, contentChangedOn } from './sitemap-dates.mjs';
...
async function assemble(opts = {}) {
  ...
  // rewiring it belongs to whoever next touches the sitemap. What matters is
  // that both go through `lib/sitemap-dates.mjs`, so there is one DEFINITION
  // of "changed" (addictedtoai-8ho) even while there are two callers of it.
  ...
  return {
     ...
     * by construction — `scripts/verify-surfaces.mjs` measures that they are.
     */
