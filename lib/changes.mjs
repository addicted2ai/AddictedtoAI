 ...
 *     judgment is visibly separate. A reviewed historical correction may carry
 *     a `correction` payload; the reader applies that payload to the displayed
 *     classification while retaining the original line and its annotation.
 *
...
export function describeChange(line) {
  ...
  /*
   * A departure the publisher replaced, never "retired" (`pulse/lib/diff.mjs`,
   * the departures section). What the Pulse observed is exactly what this says:
   * the row left the catalog and a row sharing its dated slug stem arrived in
   * the same fetch. It names the arriving row and stops there — it does not
   * claim the two checkpoints are one model, which is a judgment about the world
   * and not a thing two snapshots can settle.
   *
   * The successor's own display name, falling back to its row id, because a row
   * id is what the data guarantees and a name is what a reader can use. A
   * substitution line missing `successors` altogether — hand-written, or
   * malformed — still says something true rather than rendering as the bare kind.
  */
  if (line.kind === KIND.SUBSTITUTION) {
    const successors = Array.isArray(line.successors) ? line.successors : [];
    const names = successors
      .map((s) => {
        if (!s || typeof s !== 'object' || Array.isArray(s)) return null;
        const name = s.display_name || s.row_id;
        return typeof name === 'string' && name !== '' ? name : null;
      })
      .filter(Boolean);
    return names.length > 0 ? `replaced by ${names.join(', ')}` : 'replaced';
  }
  /*
...
export function changedFeed(lines, opts = {}) {
  ...
  for (const l of lines) {
    ...
    // `data/changes.jsonl` is append-only. A reviewed correction therefore
    // stays a new annotation line, and this reader is the one place that
    // applies its supported historical reclassification to the displayed line.
    // The raw change key and the annotation remain intact. Later corrections
    // for the same key win by file order, matching append-only chronology.
    const correction = l.correction;
    if (
      correction &&
      typeof correction === 'object' &&
      !Array.isArray(correction) &&
      correction.kind === KIND.SUBSTITUTION
    ) {
      corrections.set(l.annotates, {
        kind: correction.kind,
        successors: correction.successors,
      });
    }
  }
  ...
  const feed = lines
    ...
    .map((l) => {
      const displayed = corrections.has(l.key) ? { ...l, ...corrections.get(l.key) } : l;
      const entry = index.get(`${displayed.source}|${displayed.row_id}`) ?? null;
      return {
        date: displayed.date ?? null,
        key: displayed.key ?? null,
        kind: displayed.kind ?? 'change',
        title: displayed.display_name ?? displayed.excerpt?.title ?? displayed.row_id ?? '(unnamed row)',
        detail: describeChange(displayed),
        entry: entry ? { id: entry.data.id, url: entry.url, name: entry.data.display_name } : null,
        source: displayed.source ?? null,
        source_url: displayed.source_url ?? displayed.item_url ?? null,
        item_url: displayed.item_url ?? null,
        seeded: Boolean(displayed.seeded),
        annotations: (annotations.get(displayed.key) ?? []).sort((a, b) =>
