 ...
 */
export async function changeKindsReportStep() {
  const lines = await readChanges();
  const counts = unrecognisedKinds(lines);
  if (counts.size === 0) {
    process.stdout.write(`prebuild: change-kinds — ${lines.length} committed line(s), every kind declared\n`);
    return;
  }
